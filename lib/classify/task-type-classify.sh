...
#
# The second axis of the verification gate. lane-classify.sh answers
...
#
# New-type precedence rationale: incident first (alert language must never fall
...
task_type_classify() {
  ...
  # 1. incident: reactive triage of a fired signal.
  ...
  # 2. learning: study material -> workbook/self-check.
  ...
  # 3. planning: schedule-anchored prioritization. Tight anchors so
  ...
  # 4. operate: a recurring, defined procedure run.
  ...
  # 4b. absorb guard: absorbing external skill/command mechanics into the kit
  ...
  # 6b. review: standalone review of a CODE artifact. Acting on

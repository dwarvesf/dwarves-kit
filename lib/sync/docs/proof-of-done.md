...
| 2026-07-16 | kit move: `tests/test-board-sync.sh` + `_meta/board sync` from ops-toolkit | 0 | engine relocated to dwarves-kit `lib/board/sync/`, front door = `board sync` verb + per-repo `_meta/sync.toml`; per-board state slugs with flat-state migration; 44 pytest green via the kit wrapper; live chain (shim → bin/board → engine + config) verified |

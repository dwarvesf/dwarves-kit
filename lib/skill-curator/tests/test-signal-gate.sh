...
# regression on one half of the pattern can't hide). Mock the model via SKILL_CURATOR_REVIEWER_CMD; a
...
reset(){ rm -rf "$SKILL_CURATOR_PROPOSALS_DIR" "$SKILL_CURATOR_STATE_DIR" "$INVOKED" 2>/dev/null; }
...
keeps(){
  ...
  SKILL_CURATOR_SIGNAL_GATE=true SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$TMP/p.json"
...
SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$TMP/p.json"
...
reset
pay "$NO_MARK" > "$TMP/p.json"
SKILL_CURATOR_SIGNAL_GATE=true SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$TMP/p.json"; rc=$?
...
pay "$NO_MARK" > "$TMP/p.json"
SKILL_CURATOR_SIGNAL_GATE=true SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$TMP/p.json"
if [[ -z "$(find "$SKILL_CURATOR_PROPOSALS_DIR" -name SKILL.md 2>/dev/null)" ]]; then ok "gated session stages nothing"; else no "gated session staged a draft"; fi
...
SKILL_CURATOR_SIGNAL_GATE=true SKILL_CURATOR_SIGNAL_MARKERS="deploy" SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$TMP/p.json"
...
SKILL_CURATOR_SIGNAL_GATE=true SKILL_CURATOR_REVIEWER_CMD="$MOCK" bash "$RUN" "$PE"

...
if [[ "$CC_SI_PROPOSALS_DIR" != "$CC_SI_SKILLS_DIR" ]]; then ok "proposals != skills (CC does not auto-load proposals)"; else no "proposals == skills"; fi
...
CC_SI_REVIEWER_CMD="cat $TMP/env.json" bash "$RUN" "$PAY"
if [[ -f "$CC_SI_PROPOSALS_DIR/normal-skill/SKILL.md" && -z "$(find "$CC_SI_SKILLS_DIR" -type f 2>/dev/null)" ]]; then
...
CC_SI_REVIEWER_CMD="cat $TMP/evil.json" bash "$RUN" "$PAY"
escaped="$(find "$CC_SI_SKILLS_DIR" -type f 2>/dev/null)"
outside="$(find "$TMP" -name SKILL.md 2>/dev/null | grep -v "$CC_SI_PROPOSALS_DIR" || true)"
if [[ -z "$escaped" && -z "$outside" && -n "$(find "$CC_SI_PROPOSALS_DIR" -name SKILL.md 2>/dev/null)" ]]; then
...
if ! grep -qE '"?\$CC_SI_SKILLS_DIR' "$DIR/lib/reviewer-run.sh"; then ok "reviewer-run.sh does not reference skills dir as a target"; else no "reviewer-run.sh references skills dir"; fi

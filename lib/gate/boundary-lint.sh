...
#
# PATH: lib/gate/, lib/reflect/, commands/, tests/, kit.toml for a hardcoded dotfiles or
# ops-toolkit path -- the shape behind the PR #554 regression (a kit test reached into the
# operator's dotfiles by absolute path); tests/test-weekend-batch.sh already pins the same
# `dotfiles/home` pattern as its own precedent.
#
...
done < <(grep -rnE "$path_re" \
  "$ROOT/lib/gate" "$ROOT/lib/reflect" "$ROOT/commands" "$ROOT/tests" "$ROOT/kit.toml" 2>/dev/null \
...
while IFS= read -r hit; do
  [ -n "$hit" ] || continue
  flag "consumer skill named directly: $hit"
done < <(grep -nE "$name_re" "${name_files[@]}" 2>/dev/null | sed "s#$ROOT/##" || true)

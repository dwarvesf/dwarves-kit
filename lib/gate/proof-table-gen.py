#!/usr/bin/env python3
"""proof-table-gen.py -- generate the table-first proof-of-done confirmation
...
the `outcome()`/`outcome_read()` in lib/gate/gate-ledger.sh): a start/end pair per
...
that shape merged). Additive-tolerant: an entirely absent OUTCOME marker degrades the
...
def _kit_lib_root():
    """The REAL repo root (parent of this module's lib/ dir). Decouples helper-script
...
def render(rid, ledger_path, kit_root, lane, gate_rows, outcomes):
    ...
    out.append(
        ...
        "run-table, not the canonical `proof-of-done.md` (generators write "
...
def main(argv):
    ...
    # rid is caller-controlled; normalize it before it touches ANY path.
    raw_rid = argv[1]
    ...
    # The ONLY tree this generator may write into. realpath resolves symlinks +
    ...
    # Hard backstop (AC5): refuse the canonical filename regardless of
    ...
    # Confine the FINAL resolved out-path under docs/verification/generated/. Enforced
    ...
    # Write to the ALREADY-RESOLVED path (not the unresolved out_path), and open the

...
|---|---|---|
| `reflect propose` | `propose.py` | The cross-run distiller (SPEC-195). A retro one layer up from `/kit:retro`: that reads one run, this reads many runs of ledger telemetry and proposes backlog rows. |
...
| `reflect drain` | `drain.py` | The staging review (SPEC-196). Renders what is staged, grouped by Home. Expires anything past the window. |
| `reflect debt <list\|collect\|mark-paid>` | `weekend-batch.sh` | The debt paydown (SPEC-126, ADR-0031). Reads `\| DEBT \|` markers off the gate ledger, surfaces the week's waved + deferred items, closes one with `mark-paid`. |

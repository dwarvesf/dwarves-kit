...
| cloud | Build (Execute) | cloud-session support: the provisioning engine + the two `CLAUDE_CODE_REMOTE`-gated hooks. Assigned Build because it prepares the environment a build runs in; it produces no Shape/Watch/Check artifact. Engine `lib/cloud/`, verb `cloud <verb>`, per-repo `[cloud]` config. |

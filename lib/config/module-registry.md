...
(`install.sh`, 13 modules) has exactly one row. `team_mode` is excluded from
...
| cloud | Build (Execute) | cloud-session support: the provisioning engine + two hooks, each gated on `CLAUDE_CODE_REMOTE` AND its own switch (`CLOUD_PROVISION` / `CLOUD_DASH_GUARD`), because hooks.json is not module-filtered. Assigned Build because it prepares the environment a build runs in; it produces no Shape/Watch/Check artifact. Engine `lib/cloud/`, verb `cloud <verb>`, per-repo `[cloud]` config. |

...
| BACKLOG_STAGE_AUTO | env-only | off | [impl] | board, reflect | Opt-in for writing the staging buffer as a SIDE EFFECT. Unset (the default): `hooks/backlog-stage.sh` exits at once, so the SessionEnd harvest, the SessionStart intake sweep, and the SessionStart `N backlog candidates staged` line all stay silent; `reflect propose` (ledger window and `--retro`) prints its blocks on stdout and writes nothing. `1`/`true`/`yes`/`on` restores the staging write for all of them. Env-only because hooks never read `kit.toml`. Explicit proposer verbs the operator runs on purpose (`wrap stage`, `session intel propose`, `session audit triage`, `stats anomalies --propose`) are unaffected. |
...
| Env var | kit.toml key | Default | Status | Module | Doc |
|---|---|---|---|---|---|
| - | gate.proof_of_done | `false` | [impl] | gate | `hooks/ship-gate.sh` proof-of-done check (ADR-0025), read through `lib/gate/gate-policy.sh`. Opt-in: on only when the key resolves to `true` (project `.kit.toml` > operator overlay > kit root). Off, the hook logs `OFF-BY-CONFIG` and passes. The `docs/verification/README.md` marker still has to exist for the gate to engage at all. A project `false` over an operator `true` applies once `.kit.toml` is committed and clean. |
...
| - | wrap.distill | `true` | [impl] | wrap | The distill switch. `true` runs both halves on every wrap: the landing half (steps 0 to 6, 8, 9: board rows, commit, merge, deploy check, tidy, pull, activity line, retro, report) plus the distill half (the pre-step-0 candidate scan, both step -1 seams, all of step 7 including the drain). Ships on because operators asked for it every session; a plain `/kit:wrap` still lands the session either way, the knob only decides whether the distill half also runs. `false` (set in the operator `kit.toml` for landing-only) skips the distill half, reporting `**Built:** SKIPPED: distill off` and `**Seam:** SKIPPED: distill off`. The argument `distill` on the invocation (`/kit:wrap distill [repo...]`), or an ask that says to distill, turns the distill half on for that one run regardless of the knob. While the half is off, `wrap.before`, `wrap.after`, `wrap.build_candidates`, `wrap.build_lanes`, and `wrap.drain_staged` have nothing to govern. Resolved with `kit_config_get_root`, same fence as the autonomy knobs below: the distill half writes to home repos. |
...
| - | wrap.delete_merged_remote_branches | `true` | [impl] | wrap | Step 5 origin sweep. `true` lets `wrap apply --apply` delete, in one `git push` per 100 names with each delete leased to the tip it read, every GitHub origin branch whose same-repo PR merged at that exact origin tip, never the default branch and never a head or base of an open PR; `false` prints `N merged branches left on origin (wrap.delete_merged_remote_branches=false)` and deletes nothing. Resolved with `kit_config_get_root`, same fence and reason as the `merge_own_prs` row: it authorizes a write to origin. |
...
| Key | Kind | Filled by |
|---|---|---|
| wrap.before | skill | the operator, for a skill that must read the pre-landing working tree |
...
| wrap.build_lanes |
| wrap.carry_stray_lines |
| wrap.delete_merged_remote_branches |
| wrap.distill |
| wrap.drain_staged |
| wrap.follow_through |
| wrap.merge_own_prs |

...
| - | wrap.delete_merged_remote_branches | `true` | [impl] | wrap | Step 5 origin sweep. `true` lets `wrap apply --apply` delete, in one `git push origin --delete` per 100 names, every GitHub origin branch whose same-repo PR merged at that exact origin tip, never the default branch and never a head or base of an open PR; `false` prints `N merged branches left on origin (wrap.delete_merged_remote_branches=false)` and deletes nothing. Resolved with `kit_config_get_root`, same fence and reason as the `merge_own_prs` row: it authorizes a write to origin. |

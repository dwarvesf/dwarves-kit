...
| ID-005 | Multi-goal state: per-goal files so concurrent goals don't conflict (registry contract in SPEC-005; `/user:goals` command + rendering deferred to SPEC-006) | item b | SPEC-005 + SPEC-006 | full | SPEC-005 contract shipped (see CHANGELOG); SPEC-006 part (`/user:goals`) pending |
| ID-006 | Backlog as the canonical active session-start queue (schema in SPEC-005; `/start`-`/next` rendering in SPEC-006) | item c | SPEC-005 + SPEC-006 | normal | SPEC-005 schema shipped (see CHANGELOG); SPEC-006 part (`/start`-`/next` rendering) pending |
...
- ID-005/006 (state, the SPEC-006 command + rendering parts) land before ID-007/008 (the loop that reads that state).

...
| ID-032 | SPEC-029 fallout: the `/user:` sweep + namespace guard missed non-`.md` surfaces. (1) sweep `install.sh` echoes to bare `/<cmd>` (the bash-install path resolves bare) and the hooks' SUGGEST/echo strings to a form-agnostic command name (no slash; two install paths, no single slash form fits both); (2) extend the SPEC-029 meta-test guard to scan `install.sh` + `hooks/*.sh`, not only `*.md`, so the install/runtime surfaces it claims to cover are actually checked | this session 2026-05-22: SPEC-029 (PR #9) shipped with `install.sh` + hooks still emitting `/user:`; the guard globs `*.md` only so it never saw them (re-open-shipped, ID-025 class) | SPEC-030 | normal | executing |
Dependency notes:

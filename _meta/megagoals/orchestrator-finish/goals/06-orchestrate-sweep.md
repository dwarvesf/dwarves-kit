...
**Proof:** one run-table per item. ID-095 owes a named negative control (a secret-bearing surface): assert old streams are pruned and the cap holds. ID-096/098 = a `Done =` evidence line each. Rung 2 for ID-095, rung 1 for the other two.
**Design:** obvious
...
- **ID-095**, `.orchestrate/*.stream.jsonl` gets a retention cap (by count and/or age). The transcript streams, which can carry secrets, no longer grow unbounded; old ones are pruned.
...
Three papercuts gone. No unbounded secret-bearing stream, no dispatch on a typo'd model tier, no orphaned tmux windows after a clean run. Each fix is small, guarded, and individually verified.

## How to close the loop

Each item keeps its OWN check line (individually auditable inside this sweep):

- **ID-095:** add the retention cap to the stream-writer path; test (NEGATIVE CONTROL): create N+overflow streams (or age some past the cap), run the cap, assert overflow/old are pruned AND a secret-shaped line in a pruned file is gone. Run-table row.
...
**Done =** all three land with their own captured check: (a) stream.jsonl retention cap prunes overflow/old (negative control run-table), (b) `_route` rejects an off-allowlist `Model:` pre-flight, (c) happy-path exit kills its tmux window.

...
4. **Tier 2, delta only, model-read.** ONLY when step 3 produced deltas: dispatch ONE general-purpose subagent with the delta list. Its scope, exactly: read each delta feature's definition file, add/remove its section 5 path-index line (matching the existing line grammar: `entry -> ... -> terminal`, trigger mark from the registry), and re-place the feature on the section 2 flow-topology and section 3 system-topology diagrams at its real attach point. It must invent no edge the feature's definition does not show (regeneration rule 5 of workflow-paths.md). Zero deltas = zero dispatch; report CLEAN and stop.

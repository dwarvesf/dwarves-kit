...
| Apply mechanics | no code edit lands in this repo. Each FIX becomes one row appended to the `_meta/BACKLOG.md` of the CONSUMER repo that owns the site's source, in the shared kanban format (`\| ID \| Item \| Notes & source \| Status \|`, status `queued`). The board CLI has no add verb, so the row is written directly; `bash "$KIT/bin/board" board --backlog-file <path>` renders it back and `board set <ID> <state>` moves it afterwards. Plus a report listing every verdict. UNSURE items go in the report for the operator, never into a row. |

---
name: topology-drift
description: Maintainer-only. Use to audit THIS KIT's OWN feature estate against its path map, "run the topology-drift loop", "feature inventory drift". NOT for a consumer/adopter repo, NOT for a target project (/kit:feature-map), NOT for regenerating the registry, NOT for whole-doc prose drift (doc-drift), NOT for one known-missing feature.
...
Dwarves-kit repo dev only. Also fires on "is the workflow path map still complete", "does every feature appear in workflow-paths", or after a batch of merges that added/removed commands, agents, skills, or hooks. An audit-loop instance (docs/patterns/audit-loop.md), sibling to `/kit:feature-map`, which is the tool for auditing a target project instead of this repo's own registry.

Cross-checks the generated `docs/FEATURES.md` registry against the `docs/workflow-paths.md` path index both directions, then re-places only the DELTA features on the topology diagrams. REFUSES to run while `FEATURES.md` is stale (regenerate first). This schema is dwarves-kit-specific: a consumer/adopter repo has nothing to check against here. The registry itself is regenerated by `lib/registry/feature-registry.sh`, pinned by test-meta, never by this skill. A known line to add for one known-missing feature does not need this skill; just add it.

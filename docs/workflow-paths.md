...
| `[H/I] /kit:census -> research-features per module (parallel) -> docs/specs/<module>.md + top-level checklist` |
...
| `[D] research-features <- /kit:census -> per-module feature inventory (+ parity contract if porting) -> spec file written (read-only)` |

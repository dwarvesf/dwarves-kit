...
| `[H/I] /kit:census -> research-census per module (parallel) -> docs/specs/<module>.md + top-level checklist` |
...
| `[D] research-census <- /kit:census -> per-module feature inventory (+ parity contract if porting) -> spec file written (read-only)` |

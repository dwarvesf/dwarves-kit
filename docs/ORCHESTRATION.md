...
| Opt-in side-flows | 8 | `/kit:design`, `/kit:devs-team`, `/kit:visual-team`, `/kit:ui-design`, `/kit:test-plan`, `/kit:review-team`, `/kit:absorb`, `/kit:kit-health` |
...
render** the queue + drafts. `/kit:assign` is the **only mutator**: it writes a goal
draft, flips a backlog status, and hands off. No other entry point mutates state on intake.
`/kit:assign` accepts either an `ID-NNN` or **freeform intent** (the freeform front door,
SPEC-026): given freeform it delegates the crystallize interview to `/kit:think`, then
...
```text
  session start
       │
       ▼
  /kit:start ........... DETECT: render the BACKLOG Active queue + active goal drafts
       │                  (read-only; suggests the next command)
       ▼
  /kit:assign <ID|free> .. MUTATE: goal-craft a draft (.claude/goals/<slug>.md);
  ...
  └───────┬──────┬─────────────┬──────────────┬─────────────┬─────┘
          │      │             │              │             │
          │      ▼             ▼              ▼             ▼
          │  /kit:spec    /kit:think    /kit:debug   review code,
          │      │         /kit:spec        │          write AGENTS.md
          │      │         /kit:spec-validate│          /CLAUDE.md/specs
          │      │             │              │          (no app code)
          │      ▼             ▼              │             │
          │  /kit:execute (verification pipeline, Section 5.3)      │
          │      │             │              │             │
          │      ▼             ▼              ▼             │
          │  /kit:review  /kit:review-team  (root cause   │
          │      │             │              recorded,     │
          │      ▼             ▼              fix verified,  │
          │  (normal)      /kit:docs         human-confirm)│
          │      │             │                            │
          │      │             ▼                            │
          └──────┴────────▶ /kit:ship ◀────────────────────┘
                              ...
                              ▼
                           /kit:retro (full lane) -> docs/retro/RETRO-YYYY-MM-DD-<slug>.md
```

Opt-in beats (Section 6) slot in along this path: `/kit:design` between think and spec;
`/kit:devs-team` + `/kit:visual-team` before the spec hardens; `/kit:test-plan` before
execute; `/kit:ui-design` for downstream UI work.
...
stop holds. Wired from the backlog by `/kit:assign`, activated by whatever loop primitive is
present.

- **Trigger**: an objective handed to an activator: the built-in `/goal`, the `ralph-loop`
  plugin, or the `goal-craft` skill. `/kit:assign` writes the draft and surfaces its body;
...
### 5.2 Debug loop (`/kit:debug`, the `bug` lane)

A systematic four-phase loop under one iron law. Off-cycle: a bug-lane entry point, not a
linear phase.

- **Trigger**: `/kit:debug` on a defect, regression, or failing test.
...
The core build loop: `/kit:execute` dispatches one worker per task, verifies each in a fresh
...
- **Trigger**: `/kit:execute` on a `VALIDATED`/`APPROVED` spec on a feature branch.
...
- **Trigger**: operator says "also do Y" / "amend the spec" mid-`/kit:execute`.
...
- **Resume**: `/kit:next` (picks the next undone `- [ ]` row, skips done rows), **not** a fresh
  `/kit:execute` (which re-presents the whole plan). See `WORKFLOW.md` for why.
...
| 6 | `/kit:review-team` | PR-grade review; 3 lenses (security/architecture/test-coverage) in parallel | `REVIEW.md` (+ `TODOS.md`) | SHIP / FIX THEN SHIP / DO NOT SHIP |
...
`/kit:assign` detects the goal-loop activator (`/goal` -> `ralph-loop` -> `goal-craft`). If none
...
Re-running `/kit:assign` for an ID that already has a `.claude/goals/<slug>.md` **re-surfaces the
existing draft** instead of duplicating it or double-advancing status. The filesystem is the
source of truth.

### 7.6 DO-NOT-SHIP gate
`/kit:ship` reads `REVIEW.md` first. `DO NOT SHIP` -> **stop**, fix first. `FIX THEN SHIP` ->
...
companion docs per the WORKFLOW doc-impact map). `/kit:ship` and `/kit:retro` review that log
...
| `/kit:assign <ID-NNN or freeform>` | goal draft + lane routing (freeform: delegate crystallize to `/kit:think`, then allocate ID + BACKLOG row) | draft written, status flipped, handed off | none (mutator; idempotent; freeform gated by approve-before-allocate) |
...
| `/kit:review[-team]` | review | verdict recorded in `REVIEW.md` | advisory |
...
- `MANUAL.md` - per-command operator detail (reads/writes/gotchas for each `/kit:*`).

...
| `/kit:dispatch` | `[H/I]` | Fire several disjoint VALIDATED specs concurrently, each in its own worktree, then converge. Cross-goal fan-out behind a disjointness gate … | SPEC-002, SPEC-016, SPEC-017 +82 | test-advisor.sh, test-agent-effectiveness.sh, test-attempt-state.sh +30 |
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +200 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +73 |
...
| `/kit:mega` | `[H/I]` | Turn a multi-objective destination into a sequenced roadmap of dependent sub-goals: decompose, front-load every clarification once, set the… | SPEC-034, SPEC-036, SPEC-088 +35 | proof-loop-09-scenario-b.sh, test-advisor.sh, test-bin-forwarders.sh +21 |
...
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +170 | test-bin-forwarders.sh, test-break-it.sh, test-codex-hooks.sh +48 |

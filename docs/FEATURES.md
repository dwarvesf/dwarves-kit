...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +189 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +66 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +85 | run-all.sh, test-advisor-ledger-emit.sh, test-advisor.sh +34 |

...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Use when the operator wants to shape HOW to build after the idea passed /… | SPEC-003, SPEC-004, SPEC-005 +114 | test-command-emit-sweep.sh, test-command-triggers.sh, test-design-record.sh +21 |
...
| `/kit:dispatch` | `[H/I]` | Fire several disjoint VALIDATED specs concurrently, each in its own worktree, then converge. Cross-goal fan-out behind a disjointness gate … | SPEC-002, SPEC-016, SPEC-017 +88 | test-advisor.sh, test-agent-effectiveness.sh, test-attempt-state.sh +36 |
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +217 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +75 |
...
| `/kit:execute` | `[H/I]` | Autonomous spec execution with verification. Dispatches worker subagents per task, verifies each with kit:task-verifier, retries fixable fa… | SPEC-001, SPEC-003, SPEC-004 +63 | test-break-it.sh, test-gate-vocab-recording.sh, test-hooks.sh +9 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +97 | run-all.sh, test-advisor.sh, test-agent-effectiveness.sh +37 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +109 | test-adopt.sh, test-advisor.sh, test-agent-effectiveness.sh +41 |
| `/kit:ship` | `[H/I]` | Ship: review gate, tests, version bump, changelog, conventional commit, docs update, PR. Complete pipeline from done to merged. | SPEC-001, SPEC-002, SPEC-003 +83 | test-adopt.sh, test-board-mirror.sh, test-board-writeback.sh +32 |
| `/kit:spec-validate` | `[H/I]` | Adversarial review of a spec before implementation. 7 specialist lenses attack the spec from different angles (6 advisory, 1 blocking on th… | SPEC-002, SPEC-003, SPEC-004 +64 | test-command-emit-sweep.sh, test-design-record.sh, test-every-step-review.sh +7 |
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +180 | test-bin-forwarders.sh, test-break-it.sh, test-codex-hooks.sh +48 |
| `/kit:start` | `[H/I]` | Detect project state and suggest the right next command. The entry point for any session. | SPEC-002, SPEC-003, SPEC-004 +49 | test-board-atomic-mint.sh, test-command-emit-sweep.sh, test-config-stamp.sh +31 |
...
| `/kit:wrap` | `[H/I]` | The session-scoped landing step after ship: flips board rows, merges green PRs, checks deploys, tidies worktrees, prints the skim-first rep… | SPEC-020, SPEC-060, SPEC-072 +28 | test-bin-forwarders.sh, test-boundary-lint.sh, test-config-registry.sh +6 |
...
| `advisor` | `[D]` | adopt, battery, mega +3 | The single cross-cutting generic review lens. Runs in TWO modes at the final integration/UAT boundary -- critique (an extra uniform lens ON… | SPEC-090, SPEC-091, SPEC-092 +25 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor.sh +13 |
...
| `ship-gate.sh` | `[E]` | PreToolUse | each matrix row mapped to the run that exercised it, or an | SPEC-006, SPEC-042, SPEC-044 +54 | test-codex-hooks.sh, test-every-step-review.sh, test-gate-opt-in.sh +18 |

...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +187 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +64 |
...
| `/kit:explain` | `[H/I]` | Turn a merged change into a literate-diff explainer a human READS to understand: background -> goal + intuition -> a prose-ordered diff -> … | SPEC-050, SPEC-060, SPEC-094 +18 | proof-loop-09-scenario-b.sh, test-command-emit-sweep.sh, test-config-registry.sh +10 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +85 | test-advisor-ledger-emit.sh, test-advisor.sh, test-agent-effectiveness.sh +32 |
...
| `/kit:quiz-gate` | `[H/I]` | The ★-tap NUDGE before merging a significant+worthy gate PR: a 5-question quiz built from the ACTUAL diff+tests, handed to the operator's… | SPEC-125, SPEC-127, SPEC-136 +3 | test-install-modules.sh, test-kit-contract.sh, test-quiz-gate.sh +2 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +101 | test-adopt.sh, test-advisor-ledger-emit.sh, test-agent-effectiveness.sh +40 |
...
| `/kit:wrap` | `[H/I]` | The session-scoped landing step after ship: flips board rows, merges the operator's own green PRs one at a time, checks deploys, tidies bra… | SPEC-020, SPEC-060, SPEC-072 +10 | test-bin-forwarders.sh, test-config-registry.sh, test-config-seams.sh +3 |

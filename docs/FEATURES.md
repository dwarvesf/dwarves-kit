...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +204 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +74 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +89 | run-all.sh, test-advisor.sh, test-agent-effectiveness.sh +36 |
...
| `observe` | `[I]` | Query and render the kit's control plane from an agent session. Use when asked about the fleet's runs, gate verdicts, conformance, spend/to… | SPEC-033, SPEC-052, SPEC-128 +14 | test-bin-forwarders.sh, test-kit-contract.sh, test-runaway-guards.sh |

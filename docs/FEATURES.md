...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +173 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +54 |
...
| `/kit:execute` | `[H/I]` | Autonomous spec execution with verification. Dispatches worker subagents per task, verifies each with task-verifier, retries fixable failur… | SPEC-001, SPEC-003, SPEC-004 +57 | test-gate-vocab-recording.sh, test-hooks.sh, test-kit-contract.sh +8 |
...
| `/kit:gauntlet` | `[H/I]` | Probe-convergence engine: a bounded-revise loop that converges an ARTIFACT (docs, a runbook, a spec, an API surface) toward a FIXED OUTCOME… | SPEC-226, SPEC-227, SPEC-228 +9 | test-meta.sh |
...
| `recheck-verifier` | `[D]` | execute | Fresh-context re-audit of a right-arm verifier's PASS (task-verifier / integration-verifier / acceptance-verifier / system-verifier). RE-EX… | SPEC-090, SPEC-092, SPEC-108 +3 | proof-loop-09-scenario-b.sh, test-every-step-review.sh, test-meta.sh +1 |
...
| `gauntlet-proof-audit` | `[I]` | Use to audit the gauntlet records, "are the ROUNDS.md records honest", "verify the proof corpus", "check each run record against its eviden… | SPEC-242 | - |
...
| `stats` | `[I]` | Query or render the state of the scattered kit/tide/tg-cleanup/learned ledgers (the dwarves-kit gate/proof/telemetry corpus, tide file-move… | SPEC-001, SPEC-182, SPEC-192 +14 | proof-loop-09-scenario-b.sh, test-bin-forwarders.sh, test-config-stamp.sh +8 |

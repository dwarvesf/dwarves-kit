...
| `/kit:adopt` | `[H/I]` | Adopt the current (or a target) repo into the dwarves-kit operate-contract: inject AGENTS.md + a CLAUDE.md loader + a WORKFLOW pointer + th… | SPEC-004, SPEC-013, SPEC-014 +18 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-command-emit-sweep.sh +5 |
...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Explores 2-3 approaches one question at a time, holds for your approval p… | SPEC-003, SPEC-004, SPEC-005 +99 | test-command-emit-sweep.sh, test-design-record.sh, test-gate-vocab-recording.sh +18 |
...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +171 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +54 |
...
| `/kit:gauntlet` | `[H/I]` | Probe-convergence engine: a bounded-revise loop that converges an ARTIFACT (docs, a runbook, a spec, an API surface) toward a FIXED OUTCOME… | SPEC-226, SPEC-227, SPEC-228 +7 | test-meta.sh |
...
| `stats` | `[I]` | Query or render the state of the scattered kit/tide/tg-cleanup/learned ledgers (the dwarves-kit gate/proof/telemetry corpus, tide file-move… | SPEC-001, SPEC-182, SPEC-192 +12 | proof-loop-09-scenario-b.sh, test-bin-forwarders.sh, test-config-stamp.sh +8 |

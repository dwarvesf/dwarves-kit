...
| `/kit:battery` | `[H/I]` | The full independent-verification battery for a finished branch: a fresh-context acceptance verifier that RE-EXECUTES the verification comm… | SPEC-239, SPEC-244 | run-workflow.sh, test-hooks.sh |
...
| `/kit:dispatch` | `[H/I]` | Fire several disjoint VALIDATED specs concurrently, each in its own worktree, then converge. Cross-goal fan-out behind a disjointness gate … | SPEC-002, SPEC-016, SPEC-017 +74 | test-advisor-ledger-emit.sh, test-agent-effectiveness.sh, test-audit-scanner-contract.sh +29 |
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +175 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +56 |
...
| `/kit:execute` | `[H/I]` | Autonomous spec execution with verification. Dispatches worker subagents per task, verifies each with task-verifier, retries fixable failur… | SPEC-001, SPEC-003, SPEC-004 +58 | test-gate-vocab-recording.sh, test-hooks.sh, test-kit-contract.sh +8 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +81 | test-advisor-ledger-emit.sh, test-advisor.sh, test-agent-effectiveness.sh +26 |
...
| `/kit:review-team` | `[H/I]` | Parallel code review with 3 specialist lenses plus the kit-default advisor extra lens. Dispatches the security, architecture, and test-cove… | SPEC-002, SPEC-003, SPEC-007 +38 | test-advisor-ledger-emit.sh, test-advisor.sh, test-every-step-review.sh +6 |
...
| `/kit:spec-validate` | `[H/I]` | Adversarial review of a spec before implementation. 6 specialist lenses attack the spec from different angles (5 advisory, 1 blocking on th… | SPEC-002, SPEC-003, SPEC-004 +57 | test-command-emit-sweep.sh, test-design-record.sh, test-every-step-review.sh +7 |
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +153 | test-bin-forwarders.sh, test-command-emit-sweep.sh, test-config-stamp.sh +41 |
...
| `/kit:verify` | `[H/I]` | Re-run the test levels (task-verifier + integration-verifier + acceptance-verifier + system-verifier) on the current spec/branch read-only,… | SPEC-002, SPEC-003, SPEC-006 +36 | test-command-emit-sweep.sh, test-gate-vocab-recording.sh, test-kri-wiring.sh +5 |

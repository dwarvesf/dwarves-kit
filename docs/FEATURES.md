...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Explores 2-3 approaches one question at a time, holds for your approval p… | SPEC-003, SPEC-004, SPEC-005 +104 | test-command-emit-sweep.sh, test-design-record.sh, test-gate-vocab-recording.sh +19 |
...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +178 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +58 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +82 | test-advisor-ledger-emit.sh, test-advisor.sh, test-agent-effectiveness.sh +28 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +100 | test-adopt.sh, test-advisor-ledger-emit.sh, test-agent-effectiveness.sh +38 |
| `/kit:ship` | `[H/I]` | Ship: review gate, tests, version bump, changelog, conventional commit, docs update, PR. Complete pipeline from done to merged. | SPEC-001, SPEC-002, SPEC-003 +73 | test-board-mirror.sh, test-cheap-guards.sh, test-config-stamp.sh +24 |
...
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +156 | test-bin-forwarders.sh, test-break-it.sh, test-command-emit-sweep.sh +43 |
...
| `advisor` | `[D]` | adopt, battery, mega +3 | The single cross-cutting generic review lens (ADR-0028 SG-05/P5-P6). Runs in TWO modes at the final integration/UAT boundary -- critique (a… | SPEC-090, SPEC-091, SPEC-092 +21 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +12 |

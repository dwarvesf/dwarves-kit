...
| `/kit:think` | `[H/I]` | Challenge an idea before writing any spec. 6 forcing questions that reframe the product. Use when the operator brings a new idea, feature, … | SPEC-003, SPEC-004, SPEC-005 +30 | test-command-emit-sweep.sh, test-command-triggers.sh, test-config-stamp.sh +9 |
...
| `ship-gate.sh` | `[E]` | PreToolUse | each matrix row mapped to the run that exercised it, or an | SPEC-006, SPEC-042, SPEC-044 +50 | test-codex-hooks.sh, test-every-step-review.sh, test-gate-opt-in.sh +16 |

...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Use when the operator wants to shape HOW to build after the idea passed /… | SPEC-003, SPEC-004, SPEC-005 +110 | test-command-emit-sweep.sh, test-command-triggers.sh, test-design-record.sh +21 |
...
| `/kit:dispatch` | `[H/I]` | Fire several disjoint VALIDATED specs concurrently, each in its own worktree, then converge. Cross-goal fan-out behind a disjointness gate … | SPEC-002, SPEC-016, SPEC-017 +86 | test-advisor.sh, test-agent-effectiveness.sh, test-attempt-state.sh +35 |
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +210 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +75 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +94 | run-all.sh, test-advisor.sh, test-agent-effectiveness.sh +36 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +105 | test-adopt.sh, test-advisor.sh, test-agent-effectiveness.sh +40 |
...
| `/kit:spec-validate` | `[H/I]` | Adversarial review of a spec before implementation. 7 specialist lenses attack the spec from different angles (6 advisory, 1 blocking on th… | SPEC-002, SPEC-003, SPEC-004 +60 | test-command-emit-sweep.sh, test-design-record.sh, test-every-step-review.sh +7 |
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +174 | test-bin-forwarders.sh, test-break-it.sh, test-codex-hooks.sh +48 |
| `/kit:start` | `[H/I]` | Detect project state and suggest the right next command. The entry point for any session. | SPEC-002, SPEC-003, SPEC-004 +45 | test-board-atomic-mint.sh, test-command-emit-sweep.sh, test-config-stamp.sh +31 |
| `/kit:test-plan-review-team` | `[H/I]` | Parallel multi-lens adversarial critique of a spec's test plan (the ## Test plan section), with a bounded revise loop that tightens it. Dis… | SPEC-031, SPEC-052, SPEC-062 +11 | test-meta.sh, test-outcome-emit-sweep.sh |
...
| `/kit:think` | `[H/I]` | Challenge an idea before writing any spec. 6 forcing questions that reframe the product. Use when the operator brings a new idea, feature, … | SPEC-003, SPEC-004, SPEC-005 +31 | test-command-emit-sweep.sh, test-command-triggers.sh, test-config-stamp.sh +9 |
...
| `/kit:visual-team` | `[H/I]` | Parallel multi-lens critique of a visual/UI design. Dispatches 5 design lenses, merges findings, reports a verdict. Report-only, downstream… | SPEC-016, SPEC-018, SPEC-019 +11 | test-command-emit-sweep.sh, test-meta.sh |

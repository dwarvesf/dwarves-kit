...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +178 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +57 |
...
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +156 | test-bin-forwarders.sh, test-command-emit-sweep.sh, test-config-stamp.sh +42 |
...
| `/kit:wrap` | `[H/I]` | The session-scoped landing step after ship: flips board rows, merges the operator's own green PRs one at a time, checks deploys, tidies bra… | SPEC-020, SPEC-060, SPEC-072 +8 | test-bin-forwarders.sh, test-hooks.sh, test-wrap.sh |

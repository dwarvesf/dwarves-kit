...
| `/kit:adopt` | `[H/I]` | Adopt the current (or a target) repo into the dwarves-kit operate-contract: inject AGENTS.md + a CLAUDE.md loader + a WORKFLOW pointer + th… | SPEC-004, SPEC-013, SPEC-014 +19 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-command-emit-sweep.sh +5 |
...
| `/kit:battery` | `[H/I]` | The full independent-verification battery for a finished branch: a fresh-context acceptance verifier that RE-EXECUTES the verification comm… | SPEC-239 | test-hooks.sh |
...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Explores 2-3 approaches one question at a time, holds for your approval p… | SPEC-003, SPEC-004, SPEC-005 +100 | test-command-emit-sweep.sh, test-design-record.sh, test-gate-vocab-recording.sh +18 |
...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +172 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +54 |
...
| `/kit:execute` | `[H/I]` | Autonomous spec execution with verification. Dispatches worker subagents per task, verifies each with task-verifier, retries fixable failur… | SPEC-001, SPEC-003, SPEC-004 +56 | test-gate-vocab-recording.sh, test-hooks.sh, test-kit-contract.sh +8 |
...
| `/kit:gauntlet` | `[H/I]` | Probe-convergence engine: a bounded-revise loop that converges an ARTIFACT (docs, a runbook, a spec, an API surface) toward a FIXED OUTCOME… | SPEC-226, SPEC-227, SPEC-228 +8 | test-meta.sh |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +79 | test-advisor-ledger-emit.sh, test-advisor.sh, test-agent-effectiveness.sh +26 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +97 | test-adopt.sh, test-advisor-ledger-emit.sh, test-agent-effectiveness.sh +37 |
| `/kit:ship` | `[H/I]` | Ship: review gate, tests, version bump, changelog, conventional commit, docs update, PR. Complete pipeline from done to merged. | SPEC-001, SPEC-002, SPEC-003 +71 | test-board-mirror.sh, test-cheap-guards.sh, test-config-stamp.sh +24 |
...
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +151 | test-bin-forwarders.sh, test-command-emit-sweep.sh, test-config-stamp.sh +41 |
| `/kit:start` | `[H/I]` | Detect project state and suggest the right next command. The entry point for any session. | SPEC-002, SPEC-003, SPEC-004 +41 | test-command-emit-sweep.sh, test-config-stamp.sh, test-e2e.sh +27 |
...
| `advisor` | `[D]` | adopt, battery, mega +3 | The single cross-cutting generic review lens (ADR-0028 SG-05/P5-P6). Runs in TWO modes at the final integration/UAT boundary -- critique (a… | SPEC-090, SPEC-091, SPEC-092 +18 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +12 |
...
| `loop-engineering` | `[I]` | Use when the user wants to design or add a new bounded loop to the kit's own SDLC orchestration ("let's build a loop", "design a new loop f… | SPEC-209, SPEC-222, SPEC-225 +3 | test-loop-engineering-contract.sh, test-research-arch-contract.sh |
...
| `observe` | `[I]` | Query and render the kit's control plane from an agent session. Use when asked about the fleet's runs, gate verdicts, conformance, spend/to… | SPEC-033, SPEC-052, SPEC-128 +9 | test-bin-forwarders.sh, test-kit-contract.sh, test-orchestrate-wavefront.sh +1 |
...
| `stats` | `[I]` | Query or render the state of the scattered kit/tide/tg-cleanup/learned ledgers (the dwarves-kit gate/proof/telemetry corpus, tide file-move… | SPEC-001, SPEC-182, SPEC-192 +13 | proof-loop-09-scenario-b.sh, test-bin-forwarders.sh, test-config-stamp.sh +8 |

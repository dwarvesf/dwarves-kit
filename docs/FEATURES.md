...
| `/kit:battery` | `[H/I]` | The full independent-verification battery for a finished branch: a fresh-context acceptance verifier that RE-EXECUTES the verification comm… | SPEC-239, SPEC-244, SPEC-245 +3 | run-workflow.sh, test-boundary-lint.sh, test-break-it.sh +4 |
...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Use when the operator wants to shape HOW to build after the idea passed /… | SPEC-003, SPEC-004, SPEC-005 +106 | test-command-emit-sweep.sh, test-command-triggers.sh, test-design-record.sh +20 |
...
| `/kit:dispatch` | `[H/I]` | Fire several disjoint VALIDATED specs concurrently, each in its own worktree, then converge. Cross-goal fan-out behind a disjointness gate … | SPEC-002, SPEC-016, SPEC-017 +84 | test-advisor.sh, test-agent-effectiveness.sh, test-attempt-state.sh +34 |
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +205 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +74 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +90 | run-all.sh, test-advisor.sh, test-agent-effectiveness.sh +37 |
...
| `/kit:review` | `[H/I]` | Paranoid code review. Security, architecture, regressions, missing tests, edge cases. Produces actionable TODOS. | SPEC-001, SPEC-002, SPEC-003 +104 | test-adopt.sh, test-advisor.sh, test-agent-effectiveness.sh +40 |
...
| `/kit:spec-validate` | `[H/I]` | Adversarial review of a spec before implementation. 6 specialist lenses attack the spec from different angles (5 advisory, 1 blocking on th… | SPEC-002, SPEC-003, SPEC-004 +58 | test-command-emit-sweep.sh, test-design-record.sh, test-every-step-review.sh +6 |
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +172 | test-bin-forwarders.sh, test-break-it.sh, test-codex-hooks.sh +48 |
| `/kit:start` | `[H/I]` | Detect project state and suggest the right next command. The entry point for any session. | SPEC-002, SPEC-003, SPEC-004 +43 | test-board-atomic-mint.sh, test-command-emit-sweep.sh, test-config-stamp.sh +31 |
...
| `/kit:verify` | `[H/I]` | Re-run the test levels (kit:task-verifier + kit:integration-verifier + kit:acceptance-verifier + kit:system-verifier) on the current spec/b… | SPEC-002, SPEC-003, SPEC-006 +38 | test-break-it.sh, test-codex-hooks.sh, test-command-emit-sweep.sh +9 |
...
| `/kit:wrap` | `[H/I]` | The session-scoped landing step after ship: flips board rows, merges green PRs, checks deploys, tidies worktrees, prints the skim-first rep… | SPEC-020, SPEC-060, SPEC-072 +21 | test-bin-forwarders.sh, test-boundary-lint.sh, test-config-registry.sh +6 |

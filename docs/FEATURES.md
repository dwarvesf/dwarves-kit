...
| `/kit:design` | `[H/I]` | Opt-in interactive solution-design beat between /think and /spec. Use when the operator wants to shape HOW to build after the idea passed /… | SPEC-003, SPEC-004, SPEC-005 +108 | test-command-emit-sweep.sh, test-command-triggers.sh, test-design-record.sh +21 |
...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +208 | proof-loop-09-scenario-b.sh, run-all.sh, run-workflow.sh +75 |
...
| `/kit:execute` | `[H/I]` | Autonomous spec execution with verification. Dispatches worker subagents per task, verifies each with kit:task-verifier, retries fixable fa… | SPEC-001, SPEC-003, SPEC-004 +63 | test-break-it.sh, test-gate-vocab-recording.sh, test-hooks.sh +10 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +93 | run-all.sh, test-advisor.sh, test-agent-effectiveness.sh +37 |
...
| `/kit:pitch` | `[H/I]` | Assemble an outward buy-in doc from what a gated run already produced: the spec, the proof-of-done, the implementation-notes, and the gate … | SPEC-140, SPEC-141, SPEC-193 +2 | test-outcome-emit-sweep.sh, test-pitch.sh |
...
| `/kit:spec-validate` | `[H/I]` | Adversarial review of a spec before implementation. 6 specialist lenses attack the spec from different angles (5 advisory, 1 blocking on th… | SPEC-002, SPEC-003, SPEC-004 +59 | test-command-emit-sweep.sh, test-design-record.sh, test-every-step-review.sh +7 |
| `/kit:spec` | `[H/I]` | Generate a development spec from a feature idea or decision brief. Creates docs/specs/ with structured requirements. | SPEC-001, SPEC-002, SPEC-003 +174 | test-bin-forwarders.sh, test-break-it.sh, test-codex-hooks.sh +49 |
| `/kit:start` | `[H/I]` | Detect project state and suggest the right next command. The entry point for any session. | SPEC-002, SPEC-003, SPEC-004 +44 | test-board-atomic-mint.sh, test-command-emit-sweep.sh, test-config-stamp.sh +31 |
...
| `/kit:wrap` | `[H/I]` | The session-scoped landing step after ship: flips board rows, merges green PRs, checks deploys, tidies worktrees, prints the skim-first rep… | SPEC-020, SPEC-060, SPEC-072 +24 | test-bin-forwarders.sh, test-boundary-lint.sh, test-config-registry.sh +7 |
...
| `ship-gate.sh` | `[E]` | PreToolUse | each matrix row mapped to the run that exercised it, or an | SPEC-006, SPEC-042, SPEC-044 +53 | test-codex-hooks.sh, test-every-step-review.sh, test-gate-opt-in.sh +19 |

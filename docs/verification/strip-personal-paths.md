...
**Claim:** no personal `~/workspace/<owner>/...` example paths remain in the
...
leak-guard *pattern* strings (`grep ... 'workspace/<owner>'`) that document the
...
| # | Criterion | Result |
|---|---|---|
| 1 | No `~/workspace/<owner>/<repo>` example path in `lib/stats/`, `skills/`, `docs/` | PASS |
| 2 | Leak-guard grep-pattern docs (`'workspace/<owner>'` with no trailing slash) preserved | PASS |
...
|---|---|---|---|
| Leak absent in scope | `grep -rn 'workspace/<owner>/' lib/stats/ skills/ docs/` | 1 (no match) | PASS |
| Meta suite | `bash tests/test-meta.sh` | 0 | PASS (698/698) |

## Run detail

```
$ grep -rn 'workspace/<owner>/' lib/stats/ skills/ docs/
...
```
$ perl -pi -e 's{workspace/<owner>/ops-toolkit}{workspace/<owner>/ops-toolkit}g' \
    lib/stats/docs/verification/schema.md
$ grep -rn 'workspace/<owner>/' lib/stats/docs/verification/schema.md
lib/stats/docs/verification/schema.md:78:cd ~/workspace/<owner>/ops-toolkit
grep exit: 0        # leak present -> RED (check would fail), as expected

# restore the neutralized form
$ perl -pi -e 's{workspace/<owner>/}{workspace/<owner>/}g' \
    lib/stats/docs/verification/schema.md
$ grep -rn 'workspace/<owner>/' lib/stats/ skills/ docs/
...
```
grep -rn 'workspace/<owner>/' lib/stats/ skills/ docs/   # expect: no output, exit 1

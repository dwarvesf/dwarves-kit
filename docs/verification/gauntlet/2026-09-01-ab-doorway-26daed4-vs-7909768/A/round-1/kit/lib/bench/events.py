"""bench events: the runtime-neutral event protocol + adapters (data plane).

The protocol (run_start / stage_start / item / retry / stage_end / run_end) is
what every frontend renders and every runtime adapter emits. Frontends live
elsewhere (the forge TUI in the forge repo, viewer.py/report.py here); this
module owns the state machine, the demo stream, the kit-ledger adapter with the
expected-vs-actual conformance overlay, and the file helpers. Adding a new
agent runtime (codex, pi, opencode, ...) = writing one more *_to_events
adapter here; nothing downstream changes.
"""

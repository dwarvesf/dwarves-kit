...
SKILL_CURATOR_MEMORY_LEDGER="$(_expand "${SKILL_CURATOR_MEMORY_LEDGER:-}")"

# memory_ledger_count: echoes the queued-row count on stdout. If SKILL_CURATOR_MEMORY_LEDGER is unset,
...
memory_ledger_count() {
  if [ -z "$SKILL_CURATOR_MEMORY_LEDGER" ]; then
    echo "skill-curator: SKILL_CURATOR_MEMORY_LEDGER is not set -- set it to your knowledge/learning ledger path to surface staged-memory counts (see MANUAL.md)" >&2
    return 1
  fi
  if [ -f "$SKILL_CURATOR_MEMORY_LEDGER" ]; then
    grep -cE '\|[[:space:]]*queued[[:space:]]*\|?[[:space:]]*$' "$SKILL_CURATOR_MEMORY_LEDGER" 2>/dev/null || echo 0
  else
    echo 0
  fi
}
...
surface_counts() {  # echo "MEM DRAFTS SPEND" (space-separated)
  ...
  if [ -d "$SKILL_CURATOR_PROPOSALS_DIR" ]; then
    drafts="$(find "$SKILL_CURATOR_PROPOSALS_DIR" -mindepth 2 -maxdepth 2 -name SKILL.md 2>/dev/null \
  ...
  if [ -f "$SKILL_CURATOR_LEDGER" ]; then
    ...
    spend="$(jq -rs --arg c "$cut" '[ .[] | select((.ts // "")>=$c) | .total_cost_usd // 0 ] | add // 0' "$SKILL_CURATOR_LEDGER" 2>/dev/null || echo 0)"
  fi

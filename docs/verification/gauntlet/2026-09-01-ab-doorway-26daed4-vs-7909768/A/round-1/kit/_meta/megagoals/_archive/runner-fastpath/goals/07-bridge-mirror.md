...
- Remote execution (used by the convergence demo; build it here): the load plan for a remote target ships as a JSON plan file + a tiny applier invoked over ONE `ssh <mini-host>` call; the applier execs argv vectors. Card text NEVER gets templated into a shell string (the no-shell absolute applies to card text exactly as to queue text). Record the measured wall time of the batched load in the proof.

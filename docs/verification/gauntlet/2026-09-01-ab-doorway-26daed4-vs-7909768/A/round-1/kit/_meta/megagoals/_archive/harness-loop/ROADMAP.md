...
- [x] 04-surface-consolidation, the ADR-0034 target surface becomes reality in one wave: `lib/learn/` + `bin/learn`; bin/ regrouped to one `<subsystem> <verb>` grammar (session-* 5->1, add-backlog -> `board promote`, missing spec/goal/stats/mega/queue entries created); stats skill relocated; all call-sites repointed (dotfiles companion PR), zero alias shims, `auto`, PR #239, merged 04fbdaf (2026-07-12, operator hand-merge; companions dotfiles#213 + ops-toolkit#796)
- [x] 05-retro-cycle, `learn propose` (stats-window aggregate → sonnet interpret → adversarial check → cited, deduped staging) + the recurring `kit-retro-YYYY-WW` queue goal template + `[features] auto_improvement` flipped to [impl] with the SPEC-188 lint amended, `auto`, PR #243, merged b2131dc (2026-07-12, operator hand-merge)
- [x] 06-staging-drain, `learn drain` renders staging grouped + age-sorted + evidence-first; 30d rows move to `[expired]` (never deleted); promote/reject flow unchanged, `auto`, PR #241, merged d688306 (2026-07-12, operator hand-merge)
...
- [x] 08-config-surface, `bin/config list|get|explain` with provenance (env > project > kit-root > default) + status tags + the checked-in env↔key registry + drift lint, `auto`, PR #240, merged ad81d46 (2026-07-12, operator hand-merge)
- [x] 09-onboard-wizard, `/kit:onboard` interactive first-run orchestrator (detect install mode, offer adopt, module picker, consumer-knob capture, plugin-gap disclosure, welcome tour), `gate`, PR #242, merged 2ba70b6 (2026-07-12, Han's gate click)
- [x] 10-front-door-truth, README reorganized around the five legs + tables truth-matched (agents 25, skills per ADR §8) + weekly digest folded into session-intel + the ONE kit scheduler (single weekly LaunchAgent + declarative jobs list; per-job plists retire), `auto` (HELD as the final PR under gated-final; Han's click closes the mega), PR #244

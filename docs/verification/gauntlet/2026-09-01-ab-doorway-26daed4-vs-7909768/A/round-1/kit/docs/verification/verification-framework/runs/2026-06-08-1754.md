...
- Command: `git show HEAD:lib/gate/proof-ledger.sh > /tmp/old-ledger.sh && bash /tmp/old-ledger.sh check /tmp/vf-gate-split "$(git -C /tmp/vf-gate-split rev-parse HEAD)" vf-fix`
- Exit: 1
- Output (excerpt):
  ```

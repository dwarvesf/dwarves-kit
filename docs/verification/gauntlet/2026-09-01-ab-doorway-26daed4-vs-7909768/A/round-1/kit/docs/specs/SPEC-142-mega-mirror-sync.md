...
`lib/classify/lane-classify.sh deescalate` call site addition at an existing invocation point;
...
3. **The ship-time lane de-escalation nudge** (sub-goal 07, this kit, SPEC-141): an advisory
   `lib/classify/lane-classify.sh deescalate` call fired from `commands/ship.md` Step 8 when a
...
1. **Add three documentation-only mirror paragraphs to `commands/mega.md`, plus one
   `lib/classify/lane-classify.sh deescalate` call at the point `mega.md`'s Step 5 already derives
...
- **New `## Consolidate mode (remega, mirrors the skill's mode)` section**, placed after
  ...
  and the when-NOT table at kit-native scoping: reuses `lib/gate/dispatch-gate.sh` for `## Touches`
  overlap (the same checker Step 4 already names), reuses `lib/spec/spec-next.sh reserve` for the
  ...
  (`lib/goal/goal-drafts.sh archive`, `WORKFLOW.md` "Goal drafts (.claude/goals/)") as the nearest
...
- **Step 5:** one added prose paragraph + one added `bash` line
  (`bash lib/classify/lane-classify.sh deescalate "$LANE" --rid "$RID"`) right after the existing
...
`lib/gate/dispatch-gate.sh` (disjointness), `lib/spec/spec-next.sh reserve` (SPEC-128), the goal-draft
archive lifecycle (`lib/goal/goal-drafts.sh`), and `lib/classify/lane-classify.sh deescalate` (SPEC-141).
...
3. `commands/mega.md` Step 5 contains an added `bash lib/classify/lane-classify.sh deescalate ...` line
...
- Any change to `lib/classify/lane-classify.sh`, `lib/goal/mega-merge.sh`, `lib/queue/orchestrate.sh`,
  `lib/spec/spec-next.sh`, or `lib/goal/goal-drafts.sh` behavior.

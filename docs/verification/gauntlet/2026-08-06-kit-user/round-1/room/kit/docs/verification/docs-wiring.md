...
backed by a no-orphan sweep against `lib/queue/orchestrate.sh`/`lib/gate/gate-ledger.sh` live call sites, and an
...
| AC6 | `Model:` -> `--model` routing has a live call site in `lib/queue/orchestrate.sh` | PASS |
| AC7 | Token capture records via `lib/gate/gate-ledger.sh` (`\| TOKENS \|` marker) with a live call site | PASS |
| AC8 | TIER-4 close is actually invoked from the run terminal in `lib/queue/orchestrate.sh` | PASS |
...
--- AC6-9: no-orphan sweep (each documented capability has a LIVE call site) ------
  PASS AC6: Model: -> --model routing fires (live call site in lib/queue/orchestrate.sh)
  PASS AC7: token capture records via gate-ledger.sh (live call site in lib/queue/orchestrate.sh)
  PASS AC7: the TOKENS ledger marker is emitted (live call site in lib/gate/gate-ledger.sh)
  PASS AC8: TIER-4 close is actually invoked from the run terminal (live call site in lib/queue/orchestrate.sh)
  PASS AC9: a pane spawns under MULTIPLEXER=1 (live call site in lib/queue/orchestrate.sh)
...
11 of them RED while the no-orphan/AC10 checks (which test `lib/queue/orchestrate.sh`/`lib/gate/gate-ledger.sh`,
...
`lib/queue/orchestrate.sh` actually dispatches, not just that today's docs happen to match today's code.

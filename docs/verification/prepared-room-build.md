...
## Live confirmation (baked-image gauntlet round, 2026-09-01)

A full J1 omp/NeuralWatt round on the baked image: `BAKED-DONE rc=0 checker=GREEN install=omp-baked`. The probe wrote the `omp-baked` skip signal (zero npm traffic), ran DeepSeek via omp, and the doorway checker passed. Same card, same runner, the ONLY delta from the pre-bake regression smoke (which failed on the flaky bun install) is the baked toolchain. This is the enhanced container proving reliable end to end.

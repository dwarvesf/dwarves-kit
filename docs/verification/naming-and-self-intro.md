# Proof of done: naming-and-self-intro (SPEC-221)
...
|---|---|---|---|
| 1 | meta integrity + the new SPEC-221 pins | `bash tests/test-meta.sh` | 746/746 PASS, exit 0 |
...
| 5 | negative control A: banner strip | `sd -s '[kit:start]' '[kit-start]' commands/start.md; bash tests/test-meta.sh` | 745/746, exactly `commands/start.md wires the self-intro banner (SPEC-221)` FAIL; restored -> 746/746 |
...
An unplanned live catch confirms the FEATURES freshness pin discriminates too: after `docs/specs/SPEC-221-*.md` landed, `test-meta` went RED on `docs/FEATURES.md is fresh` (the spec's command mentions changed the registry's Specs-refs projection) until regeneration, exactly the drift class the pin exists for.

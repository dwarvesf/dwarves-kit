...
`feat/loop-08-config-surface` (based on `origin/docs/loop-01-taxonomy`).

## Run table

| # | Check | Command | Result |
|---|---|---|---|
| 1 | `config list` full render, kit-root defaults (no overrides in play) | `bash bin/config list` | 92 lines, header + 91 declared-key rows. Full text capture below. |
...
4 config-bootstrap vars + 33 kit.toml-only keys (12 modules + 21 other section keys) - 4
double-counted rows where an env var and a kit.toml key share a line = 91 body rows. Two rows
show `env` provenance from the runner's OWN inherited shell environment (`PROSE_RAG_INJECT=1`,
`MONEY_GATE_REPOS=family-office:trading`) -- an incidental, non-reproducible observation, not
part of the controlled proof; the controlled fixture below is the reproducible evidence.
...
TIER4_CLOSE                    [impl]     `1` (truthy)                   default              mega
MULTIPLEXER                    [impl]     `0` (off)                      default              mega
...
CC_SI_MEMORY_LEDGER            [impl]     `""`                           default              session
...
PROSE_RAG_INJECT               [impl]     1                              env                  prose_rag
MONEY_GATE_REPOS               [impl]     family-office:trading          env                  money_gate
...
winner named explicitly at the bottom.
...
=== AC5: bin/config functional smoke (list/get/explain + provenance fixture) ===
  PASS get WAVE_CAP (default, no overrides) == 2
  PASS get mega.wave_cap (dotted-key lookup) == 2

...
- Command: `bash lib/gate/verif-counts.sh && bash tests/test-meta.sh && bash tests/test-hooks.sh`
- Exit: 0
- Output (excerpt):
  ```
...
- Note: the live counts are in `docs/verification/COUNTS.md` (the GEN block), not typed
  here. To change a number, change the tests then `bash lib/gate/verif-counts.sh`.

## 2026-06-07 01:11 REGENERATE -- single-source proof (figure follows the source)
- Command: add 3 meta-test pins, then `bash lib/gate/verif-counts.sh`
- Output (excerpt):
  ```
...
- Command: `git worktree add --detach /tmp/sed2 HEAD && cd /tmp/sed2 && mv -f lib/gate/verif-counts.sh docs/verification/COUNTS.md docs/verification/README.md /tmp/ && bash tests/test-meta.sh`  (throwaway worktree; removed after; shared checkout untouched)
- Exit: 1
- Output (excerpt):
  ```
  FAIL lib/gate/verif-counts.sh exists and is executable
...
- Reproduce: `bash lib/gate/verif-counts.sh` (regenerates COUNTS.md from the live suites).

...
This category was already audited and justified in SG-03's own proof (`docs/verification/kitmod-subsystem-commands.md` §"F-bar per entry"): the header comment IS the doc, matching the pre-existing `board.sh`/`orchestrate.sh` convention. Re-confirmed here, no regression.
...
## Ship-gate back-compat run (folded from prior docs/verification/kitmod-docs.md)

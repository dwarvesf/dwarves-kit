...
`lib/classify/lane-classify.sh` escalates a machinery-lib EDIT to `full`, not a mere MENTION, when
...
- `lib/classify/lane-classify.sh`: `--files` on `classify`/`explain`/`check`; `_extract_files` +
...
| edit | `classify --files "lib/goal/mega-merge.sh" "add a guard clause"` | `full` | `full` (flags: kit-machinery) |
...
$ classify --files "lib/goal/mega-merge.sh" "add a guard clause"

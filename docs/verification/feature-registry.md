# Proof of done: feature-registry (SPEC-217)
...
`lib/registry/feature-registry.sh` (bash + grep/sed/awk/jq, LC_ALL=C, no timestamps in output) emits `docs/FEATURES.md`; `tests/test-meta.sh` gained the SPEC-217 regenerate-and-diff pin.
...
| 2 | full suite green | `bash tests/test-meta.sh` | `Passed: 733 / 733`, incl. `PASS docs/FEATURES.md is fresh (regenerate == committed, SPEC-217)` |

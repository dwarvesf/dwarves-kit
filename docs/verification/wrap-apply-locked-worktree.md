...
`feat/wrap-apply-locked-worktree`, base a861ba2 (origin/master at start). Change: `lib/wrap/wrap.sh`
`_apply_worktrees` plus `_merge_proof`, `_wt_locked`, `_wt_cleared`; tests `tests/test-wrap.sh`; doc
`commands/wrap.md` step 5.
...
Exit: 0
Output (excerpt): `test-wrap: all 350 passed` (330 before this branch; 20 new assertions across the
locked-worktree matrix, the postcondition case, and the two report-line checks)
Verdict: PASS
...
Command: `bash bin/wrap apply --worktrees /Users/tieubao/workspace/dwarvesf/dwarves-kit`
Exit: 0
Output (excerpt):
...
Re-run `bash tests/test-wrap.sh` on this branch for the matrix and the postcondition, and

...
| Where | `lib/queue/orchestrate.sh` (`_goalfile`, `_route`, `cmd_run`), `tests/test-orchestrate.sh` (TEST 6-7), `docs/specs/SPEC-087-context-hygiene.md` (routing section) |
| How it runs | `bash lib/queue/orchestrate.sh run <dir> [--dry-run]`; CLI flags `--model`/`--effort` verified present (`claude` 2.1.195) |
...
| R3 | 2026-06-29 | `bash lib/queue/orchestrate.sh run <mixed-tiers> --dry-run` | 0 | distinct tiers per sub-goal + inherit |
...
- Command: `bash lib/queue/orchestrate.sh run <mixed-tiers fixture> --dry-run`
- Output:
  ```
...
bash lib/queue/orchestrate.sh run <megagoal-dir> --dry-run
```

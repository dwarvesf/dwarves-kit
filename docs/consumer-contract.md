...
| `bin/reflect` | `lib/reflect/reflect.sh` (`reflect debt <list\|collect\|mark-paid>`, `reflect propose`, `reflect drain`; renamed from `learn` by ADR-0036, `bin/learn` kept one release as a deprecation forwarder) |
...
| `bin/wrap` | `lib/wrap/wrap.sh` (`scan` report-only; `apply [--apply] [--worktrees]` branch delete + worktree remove + `--ff-only` pull, plus the origin delete of branches whose same-repo PR merged at their exact tip (`[wrap] delete_merged_remote_branches`, root-only, default `true`), plus opt-in `--archive-unmerged` (never under `--own`): pushes every local branch not current/default/worktree-held with a commit `git cherry origin/<default>` marks unique to `origin archive/<slug>-<date>` (no force), deletes the local branch only once that push lands, dry-run by default; `merge [--apply] [--pr N]` one own green PR per call (`--pr` names exactly one own open PR, marking a draft ready first), base = default branch, mergeable, checks green, no unresolved threads, no open dependents, a draft is a SKIP line; `log` prepends a dated line to `[wrap] activity_log (from a worktree of the log's repo, the worktree's copy is written)`, operator or kit-root `kit.toml` only, path must resolve under `$HOME`; `default-branch` prints the detected name; `deploy-wait <owner>/<name> <sha> [--check <substr>]... [--timeout <secs>]` reads a SHA's check runs until every match is completed, highest id per name, exit 0 only when all concluded `success`, 1 a failure (names printed), 2 a gh or read error, 124 timeout, writes nothing; `follow-mode [lanes\|all]` prints step 10's mode and lanes from `[wrap] follow_through` (`off`, `lanes`, `all`; root-only, default `off`; any other value prints one line and runs as `off`)) |

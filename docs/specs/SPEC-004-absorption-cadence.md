...
2. **The gate**: the inline adoption rubric (score >= 10) AND the NO-list (PHILOSOPHY §3) AND the reject-list AND "serves 2+ lifecycle phases" AND **"does not duplicate an existing kit component or external tool"**. No new scoring *command*; the rubric lives as text in ABSORPTION.md.
...
- [x] **TASK-1: Author `docs/ABSORPTION.md`.** (built: commit d31eb11) Sections (pinned for TASK-4 grep): `## The external lane` (covering BOTH lanes, A = Credits drift re-audit, B = open-discovery scan), `## Interest areas` (workflow/orchestration, agents, QA/testing, UI/design, the scan foci for lane B), `## Seed list` (lane-B scan targets, defined to avoid two-source drift as the **live README Credits parse** union a **pinned list of only the non-Credits SPEC-014 extras** in this file, so Credits stays the single source for credited repos; grown AND shrunk only by maintainer edit. Pin the **since-last-run baseline**: each seed repo's last-seen HEAD SHA, `gh api repos/{owner}/{repo}/commits/HEAD --jq .sha`, recorded in the proposal's machine-readable footer; "changed" means the SHA differs from the most recent prior proposal; the first run flags all as new. Pin **ranking + cap**: rank by rubric total, tie-break by interest-area weight (agents/workflow > QA/UI), surface the top <=15 prominently, but every gate-passer (>=10 + all gates) still appears, capped-out ones go to an **overflow appendix**, never silently dropped), `## The adoption rubric`, `## The gate`, `## Cadence`, `## The proposal artifact`, `## Handoff into WORKFLOW`. State the **human merge gate**: discovery + scoring + drafting are automatic, but adopting any source/pattern (and adding a discovered source to README Credits) is maintainer-approved and flows through WORKFLOW. Note QA/UI candidates needing binaries route to "recommend external" (PHILOSOPHY section 3). Reference SPEC-014 (the one-shot wide survey this generalizes) and SPEC-016 (the agents/QA/UI lanes). Carry the rubric table inline (4 factors, 0-4, >=10 = ADOPT). State the gate (rubric + NO-list + reject-list + 2+ phases for candidates + no-duplicate). State the cadence as **maintainer-triggered, target monthly, no enforcement** (not "monthly" as if guaranteed). State that absorption proposes, never auto-merges, and that every absorbed component runs the full SDD WORKFLOW + a README citation + a §5 soak, and that a component failing its §5 soak after absorption routes to PHILOSOPHY §5's deprecation path. Note the internal lane is SPEC-007.
...
- [x] **TASK-2: `docs/absorption/` + template + README.** (built: commit d31eb11) `TEMPLATE.md` defines the proposal shape: per-candidate row (source repo+URL | lane A/B | interest area | what it is | rubric score /16 | rank | NO-list verdict | 2-phase check | duplicate check | recommendation ADOPT/ADAPT/REJECT | rationale), plus an **overflow appendix** (gate-passing candidates below the display cap, listed not dropped) and a **machine-readable footer** recording each seed repo's last-seen HEAD SHA (the lane-B since-last-run baseline). `README.md` explains the dir, the **append-suffix** collision rule (`-2`, `-3`; never overwrite, to preserve the point-in-time audit), the **last-run ledger** (each run stamps its date so staleness is visible), and that a no-drift run still writes a dated proposal with a "no candidates / no drift" body (the dir doubles as run history). State the approve -> BACKLOG -> SPEC handoff.
...
- [x] **TASK-4: Tests + cross-refs.** `tests/test-meta.sh` new block `=== Absorption ritual ===`: assert `docs/ABSORPTION.md` exists with the six pinned headers + the rubric table marker; `docs/absorption/TEMPLATE.md` + `README.md` exist; `commands/absorb.md` exists with frontmatter `description:` and contains the injection-guard string. Expected meta-count delta: **+9** (6 headers + 2 file-exists + 1 guard-string), confirm and document the new total. Update `PHILOSOPHY.md` §5 to reference `docs/ABSORPTION.md`'s rubric and **stop implying `/eval-tool` is a command**. One-line cross-reference from `README.md` + `MANUAL.md` (command row for `/user:absorb`). CHANGELOG entry.
...
- [x] `docs/ABSORPTION.md` exists with the six pinned sections + the inline rubric (>=10 threshold); cadence stated as maintainer-triggered/no-enforcement
- [x] The gate includes the no-duplicate criterion (catches internal kit duplicates, not just external tools)
- [x] No `/eval-tool` command reference anywhere; PHILOSOPHY §5 updated to point at the inline rubric
- [x] `docs/absorption/` exists with `TEMPLATE.md` + `README.md`; append-suffix collision rule; last-run ledger; empty-run record
- [x] `/user:absorb` exists, maintainer-only, connective-tissue (not 2-phase-justified), running both lanes (A Credits drift re-audit + B open discovery over the interest areas), with capability check + Credits-parse-and-validate + DATA-not-instructions guard + `git status` self-check
- [x] Discovery + scoring + drafting are automatic, but adoption (and adding a discovered source to Credits) is a human merge gate; QA/UI binary-needing candidates surface as "recommend external", not "absorb" (DEC-019)
- [x] `docs/ABSORPTION.md` has the `## Interest areas` section (agents / QA-testing / UI-design / workflow); references SPEC-014 + SPEC-016
- [x] Absorption output flows through the normal WORKFLOW; a failed-soak component routes to PHILOSOPHY §5 deprecation
- [x] `bash tests/test-hooks.sh` 42/42 (no hook touched); `bash tests/test-meta.sh` green (+9, new total documented)
- [x] No new hook, agent, dependency, env var, settings.json field, or scheduler
- [x] CHANGELOG entry; internal lane (item e) recorded as deferred to SPEC-007
...
9. **A lane-B seed repo dies / renames / goes private.** Same per-repo unreachable handling as lane A (edge 1); the pinned non-Credits extras list shrinks only by maintainer edit (it grows and shrinks by the same human gate), and the dead seed is recorded in the proposal so the maintainer can prune it.
10. **More gate-passing candidates than the display cap.** The top <=15 (by rubric total, tie-broken by interest-area weight) are surfaced prominently; every other gate-passer goes to the overflow appendix. The cap orders display, it never decides what passes, so a real ADOPT can never be silently truncated (the failure that would contradict this spec's reason to exist).
11. **First lane-B run (no prior proposal / no baseline SHAs).** Every seed repo is flagged "new" and scored; the run records the baseline SHAs for next time. Subsequent runs only re-score repos whose HEAD SHA changed.
...
- **DEC-020 (re-validation, 2026-05-21)**: The post-widening spec-validate flagged that lane B's "new/changed since last run, ranked + capped" was asserted but not mechanized. Pinned: (a) a HEAD-SHA baseline (`gh api repos/{owner}/{repo}/commits/HEAD --jq .sha`) in the proposal footer, since WebFetch has no diff; (b) the rank key (rubric total, tie-break interest-area weight) + display cap (<=15); (c) an **overflow appendix** so the gate (>=10 + all gates), not the cap, decides visibility, a real ADOPT can never be silently truncated. Also: the seed is `live Credits parse ∪ pinned non-Credits SPEC-014 extras` (kills two-source overlap drift), and the DATA-not-instructions guard extends to the ranking sink (fetched text cannot influence rank/cap position). Resolves the 3 criticals.
...
**Re-validation (2026-05-21, after the DEC-019 widening + Think/Design narrowing):** 5-lens spec-validate run on the widened lane B. Verdict NEEDS REVISION, 3 criticals, all from one root (lane B's "new/changed since last run, ranked + capped" asserted but not mechanized), resolved inline:
- **C1 baseline**: WebFetch has no since-X diff -> pin a HEAD-SHA baseline (`gh api ... --jq .sha`) in the proposal footer; "changed" = SHA differs from the prior proposal (DEC-020).
- **C2 ranking/cap unspecified**: pin cap (<=15) + sort key (rubric total, tie-break interest-area weight) in `## Seed list` + TASK-3.
- **C3 cap can hide a real ADOPT**: the cap orders display only; every gate-passer appears, overflow appendix, never dropped (edge 10, TEMPLATE overflow section).
- Warnings folded: lane-B seed drift (edge 9), two-source de-drift (seed = live Credits ∪ pinned non-Credits extras only), ranking as a new injection sink (guard extended to rank). Security base case, the human gate, the web-search cut, and approach A all PASSED.
Status returned to VALIDATED after these inline fixes.

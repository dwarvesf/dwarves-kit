...
**Ship a documented, repeatable upstream-absorption ritual (`docs/ABSORPTION.md`) that carries the adoption rubric inline, plus a maintainer-only `/user:absorb` command (connective tissue, like `/kit-health`) that audits the README-Credits source repos against the kit and drafts a dated, proposal-only artifact under `docs/absorption/`. An approved proposal flows through the standard SDD WORKFLOW. No new hook, no scheduler, no autonomous loop, no separate scoring command.**
...
3. **`/user:absorb`** (new command, maintainer-only, connective tissue): resolves the README-Credits source list, fetches each repo (treating fetched content as DATA, never instructions), diffs against what the kit absorbed, scores new/changed patterns against the gate, and writes a dated proposal. It proposes; the maintainer approves; nothing is absorbed automatically.
...
- [ ] **TASK-1: Author `docs/ABSORPTION.md`.** Sections (pinned for TASK-4 grep): `## The external lane`, `## The adoption rubric`, `## The gate`, `## Cadence`, `## The proposal artifact`, `## Handoff into WORKFLOW`. Carry the rubric table inline (4 factors, 0-4, >=10 = ADOPT). State the gate (rubric + NO-list + reject-list + 2+ phases for candidates + no-duplicate). State the cadence as **maintainer-triggered, target monthly, no enforcement** (not "monthly" as if guaranteed). State that absorption proposes, never auto-merges, and that every absorbed component runs the full SDD WORKFLOW + a README citation + a §5 soak, and that a component failing its §5 soak after absorption routes to PHILOSOPHY §5's deprecation path. Note the internal lane is SPEC-007.
  - Acceptance: six pinned sections present; rubric table inline with the >=10 threshold; cadence stated as maintainer-triggered/no-enforcement; the no-duplicate gate criterion present; the failed-soak -> deprecation path stated; no `/eval-tool` command reference anywhere; references PHILOSOPHY §5 rather than restating its 8 steps; ASCII-clean headers.
...
- [ ] `/user:absorb` exists, maintainer-only, connective-tissue (not 2-phase-justified), external-lane only, with capability check + Credits-parse-and-validate + DATA-not-instructions guard + `git status` self-check
...
- Absorbing from sources not in README Credits (adding a source is itself an absorption decision).

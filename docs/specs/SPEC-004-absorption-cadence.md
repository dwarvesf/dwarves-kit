...
3. **`/user:absorb`** (new command, maintainer-only, connective tissue): runs two lanes. **(A) Drift re-audit**, resolves the README-Credits source list, fetches each repo (treating fetched content as DATA, never instructions), and diffs against what the kit absorbed. **(B) Open discovery**, scans the wider ecosystem (the SPEC-014 survey set as the seed list, plus targeted queries per interest area: workflow/orchestration, agents, QA/testing, UI/design) for new candidate sources and patterns. Both lanes auto-score against the inline rubric + the full gate and auto-write a dated proposal. It proposes; the maintainer approves; **nothing, and no newly-discovered source, is absorbed automatically** (the human merge gate, preserving "synthesize, don't originate"). QA/UI candidates that need a binary/runtime (Playwright, visual tooling) score low on adoption-cost and surface as "recommend external" per PHILOSOPHY §3, not "absorb".

...
(`lib/gate/gate-ledger.sh`'s `logs/runs/<rid>.log`) is written on every gated run and never read
...
"single-source, generated, never hand-typed" gap `lib/gate/verif-counts.sh` already closed for
...
1. **A Python generator (`lib/gate/proof-table-gen.py`) behind a thin bash launcher
   (`lib/gate/proof-table-gen.sh`), mirroring `lib/goal/handoff-gen` -> `lib/goal/handoff/handoff_gen.py`.**
   The parsing + conditional-column table rendering is naturally a dict/list problem;
   `lib/queue/orchestrate.sh` and friends are explicitly bash-3.2-safe (no `declare -A`, the macOS
   ...
   (`lib/gate/verif-counts.sh`'s inline python3 substitution, `lib/goal/handoff-gen`, `lib/queue/orchestrate.sh`
...
3. **Extend `lib/gate/verif-counts.sh` to also emit the proof table.** Rejected: verif-counts.sh's
...
`lib/gate/proof-table-gen.sh <rid> [out-path]` resolves the kit's durable log dir the exact way
`lib/gate/gate-ledger.sh` does (sources `lib/telemetry/kit-log-dir.sh`, calls `kit_migrate_log_dir`, reads
`kit_resolve_log_dir`) so there is one resolver, not two, then execs
`lib/gate/proof-table-gen.py "$@"` with `KIT_ROOT` / `KIT_LOG_DIR` exported. The Python module:
...
- Reuses `lib/gate/gate-ledger.sh required <lane>` and `lib/gate/gate-ledger.sh check <lane> <rid>`
...
> GENERATED by `lib/gate/proof-table-gen.sh` ... Do NOT hand-edit.
...
## 4. Reproduce
`bash lib/gate/proof-table-gen.sh <rid>`
```

## Acceptance criteria

1. Given a rid with a populated ledger, `bash lib/gate/proof-table-gen.sh <rid>` writes a

...
Three keys make the write actions configurable, each defaulting to the acting posture and each resolved with `kit_config_get_root` so a project `.kit.toml` can never set it: `wrap.merge_own_prs` (step 3), `wrap.tidy_worktrees` (step 5), `wrap.build_candidates` (step 7b). A `false` turns that step's action into a report line named in `FYI`; it never turns the step off and never relaxes a refusal the tools make on their own. Proof: `docs/verification/wrap-autonomy-knobs.md`.

`commands/wrap.md` carries the eight steps with these fixed rules: step 0 defines foreign activity as a worktree reflog entry newer than the session start and an `index.lock` older than 5 s or persisting for 5 s, and the check runs again before steps 3, 5 and 6; step 3 uses `wrap merge` (one PR per call) and, when the session's open PRs form a chain, SPEC-065 order (retarget dependents first, merge parent-first); merged is not deployed, a `workflow_dispatch` repo gets the dispatch and a `headSha == merge SHA` check before `DEPLOYED` is claimed; the session's own EnterWorktree worktree is removed (`ExitWorktree remove`, `discard_changes: true`, then `git branch -D`) once `wrap scan` proves its branch squash-merged and the worktree is clean, and kept only when dirty, unproven, or without `gh`; step 5 targets the main checkout rather than the session cwd, passes `--worktrees` on every `apply` call, and closes with a final `wrap scan` that step 9 derives `Left alone` from, scoped to the pass's own residue with one count row per repo for the rest unless the `wide` argument is given; step 7 runs `/kit:retro` when a PR merged in step 3 appears in a `| GATE | ship | ran | shipping pr=#<n>` line of any run ledger under the kit log dir; `Needs you` carries an admission test, an item belongs there only when the operator is the only one who can do it (human credential or presence, irreversible and outward-facing, or a judgment with differing irreversible outcomes) and anything else is run before the report prints and reported past tense in `What happened`; `FYI` carries its own admission test, a bullet is either a state that shifted or a follow-up already captured, and the second kind names the file or board where that follow-up lives, because future work with no home is a task hiding in the read-only lane; the report block is the `## Closeout` grammar with `Needs you` first, the fixed uppercase tokens, and the sections `What happened`, `Shipped`, `Left alone`, `FYI`, the last three as markdown tables with fixed header rows (an overlay may append its own lines after `FYI`).

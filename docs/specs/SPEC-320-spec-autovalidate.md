...
3. **`/kit:spec` dispatches it.** After step 4's approval, after the full lane's devs-team and advisor fold, and after the command records `Spec ran` (so `descent` sees spec before validate), `/kit:spec` dispatches the validator and applies item 2. The reminder line is removed. In `/kit:spec` the operator is present, so on NEEDS REVISION the lead shows the folded criticals to the operator for re-approval before the re-run; warnings still fold silently.
...
- Code review: in `/kit:spec` the operator is present, so a NEEDS REVISION re-run shows the folded criticals for re-approval first; warnings still fold silently, and execute's preflight and a wrap step-10 worker still fold and re-dispatch unattended.
- Code review: last validate line wins; a newer failed validation must not be masked by an older pass.

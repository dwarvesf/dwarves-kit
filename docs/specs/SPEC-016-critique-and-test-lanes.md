...
/user:think -> /user:design (shape solution into DECISION-BRIEF.md, SPEC-011)
            -> /user:devs-team    critique the brief's ## Solution across 5 eng lenses (parallel)
                                  -> merged findings + verdict appended to the brief
...
- **`/user:devs-team`** reads `docs/specs/DECISION-BRIEF.md` (the `/user:design` output). Lenses: **simplicity**, **performance**, **boundaries/composability**, **data-model & correctness**, **operability/failure-modes**. Output: a merged critique (findings by severity, which lens caught each, one combined verdict `SOLID / REVISE / RECONSIDER`) appended to the brief as a `## Design critique` section. Read-only on code; it only edits the brief.
- **`/user:visual-team`** critiques a described/linked visual design. Lenses: **hierarchy/typography**, **system-consistency**, **accessibility/contrast**, **restraint/simplicity**, **expressiveness/brand-fit**. Output: same critique shape. Downstream-facing (see Part C).
...
| TASK-6 | `tests/test-meta.sh` + `MANUAL.md` + README + `.claude-plugin/{plugin,marketplace}.json` + CHANGELOG (count 15 -> 18) | Tests + hygiene (doc-impact map) | TASK-1..5 |

### Task breakdown

**Phase 1: The three commands**
- [ ] **TASK-1: `commands/devs-team.md`.** Frontmatter `description:`. Reads `docs/specs/DECISION-BRIEF.md`; if absent, says so and stops. Dispatches 5 read-only lenses (simplicity, performance, boundaries/composability, data-model/correctness, operability/failure-modes) as parallel Task calls; each returns findings + a 0-10 score. Merge/dedupe by severity, combined verdict `SOLID / REVISE / RECONSIDER`. Appends a `## Design critique` section to the brief. Report-only; never blocks. States the bypassPermissions caveat.
  - Acceptance: `description:` present; reads DECISION-BRIEF (greppable); 5 lenses dispatched in parallel; appends to the brief, does not touch code; verdict is report-only.
...
**Phase 3: Verify + hygiene (exercise the SPEC-006 doc-impact map)**
- [ ] **TASK-6: Tests + cross-refs.** `tests/test-meta.sh`: assert the three command files exist with `description:`. Update `MANUAL.md` inventory, README command table, `.claude-plugin/plugin.json` + `marketplace.json` (and every "N commands" string) from 15 to 18. CHANGELOG entry. Per the SPEC-006 doc-impact map for new `commands/*`.
  - Acceptance: `bash tests/test-meta.sh` passes (count rises by the documented delta); `bash tests/test-hooks.sh` unchanged (no hook touched); the three commands in the MANUAL inventory; the command count is 18 everywhere it appears; CHANGELOG entry.

## Acceptance Criteria (global)
- [ ] `/user:devs-team` critiques the decision brief across 5 eng lenses in parallel; appends a report-only `## Design critique`; never blocks
...
## Edge Cases
1. **`/user:devs-team` with no DECISION-BRIEF.md.** Say the brief is absent, suggest `/user:design` first, stop. Do not invent a design to critique.

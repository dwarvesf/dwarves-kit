...
| Apply mechanics | this repo holds no website source, so no code edit lands here. The apply is a report plus one board row per FIX on the board of the CONSUMER repo that owns that site's source (`bin/board promote`, shared kanban format). |

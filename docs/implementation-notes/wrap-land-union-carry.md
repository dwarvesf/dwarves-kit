...
Deltas from SPEC-317. Nothing here repeats what the spec already states.
...
- Why: the design decision (SPEC-317, union-carry only) means `land` never has an autonomy knob to consult for that file, so a NOTE naming a knob-gated action that will never happen would mislead rather than help.

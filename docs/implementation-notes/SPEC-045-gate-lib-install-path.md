...
- End-to-end consumer-gate repro recorded in `docs/verification/gate-lib-install-path.md`: lib-unreachable -> exit 0 (fail open, the bug); lib-via-install -> exit 2 (BLOCK, the fix) with the SPEC-044 type-aware message; proof added -> exit 0 (PASS).

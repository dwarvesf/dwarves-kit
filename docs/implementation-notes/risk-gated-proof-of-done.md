...
- Context: `lib/classify/lane-classify.sh` already maps a description to tiny|normal|full|bug|backfill. Han's axis is "behavior change / deploy / migration / data" vs "docs/cosmetic".
- Decision/Change: add `lib/gate/proof-gate.sh` mapping a description to one of three PROOF classes: `stateful` (deploy/migration/data/persistent), `behavioral` (implementation that changes behavior), `inert` (docs/comments/cosmetic/pure text). It reuses `lane-classify.sh` (tiny/backfill -> inert) and adds a stateful keyword pass.

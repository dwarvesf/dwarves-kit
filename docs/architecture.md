...
|---|---|---|
| context-kit (horizontal) | data: the user's context tree (`self/ people/ projects/ topics/ journal/`), recall, export | requires nothing; fills `[knowledge] root` (through `ctx adopt`) and puts `ctx` on PATH, whose `index`/`query`/`hook` verbs `bin/prose-rag` resolves first (a `prose-rag` binary stays a valid fallback); the vendored engine copy is gone |

...
5. **Name + shape the reviewers consistently (both arms).** The left/right review functions today wear many names across two forms (`-validate` / `-team` / `-checker` / `-verifier` / `-auditor` / bare) , SG-06 was about to add three more. Adopt ADR-0029: `<x>-reviewer` (left static) / `<x>-verifier` (right dynamic) / `<x>-team` (panel command) / `advisor` (generic); every review is an agent, `-team` commands orchestrate them. SG-06's new agents are born under it (free); a gated one-time rename of the 3 legacy names (`integration-checker`, `reviewer`, `security-auditor`) is SG-08.

These add THREE sub-goals to the wave (SG-06: right-arm parity + meta-agent scaffolding; SG-07: lane-escalation; SG-08: review-naming migration per ADR-0029) and reshape SG-05 (over-suggest -> generic advisor). See the kit-hardening mega-goal ROADMAP.

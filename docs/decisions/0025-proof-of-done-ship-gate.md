...
3. **Opt-in per repo.** The gate engages only where the convention is adopted (`docs/verification/README.md` exists). A repo that never adopted proof-of-done is never gated (fail open). This keeps a globally-installed hook from blocking unrelated repos. *Amended 2026-09-16:* `/kit:adopt` always writes the marker, so for adopters the gate was effectively mandatory. An adopted repo now switches it off with `proof_of_done = false` under `[gate]` in its `.kit.toml` (read through `lib/gate/gate-policy.sh`; the hook logs `OFF-BY-CONFIG`). The marker stays the adoption signal; the key is the opt-out. The same block carries `lane_gates`, `understanding_gate`, and `commit_format`; the safety gates have no key. *Amended again 2026-09-16 (later the same day):* the four keys now default to `false` in the kit root, so the gates are OPT-IN. The marker plus `proof_of_done = true` (in the repo's `.kit.toml` or the operator overlay) is what engages this gate; `adopt` seeds the block with each key explained so a new adopter sees where to turn it on.

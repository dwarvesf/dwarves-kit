...
`skills/web-drift/SKILL.md` (SPEC-232) points the shape OUTWARD, at live public websites rather than at any checkout: item set = the sites in `WEB_DRIFT_SITES` (a consumer knob with no default, so the kit ships no hostname); contract = the site stays discoverable and readable by an agent that runs no JavaScript; evidence = live HTTP responses from `lib/webcheck`; apply = board rows in whichever repo owns the site's SOURCE, since this repo holds no website. It is the one instance where REMOVE has no meaning (a live site is never superseded by an artifact in here) and the one that does NOT dispatch `audit-scanner` (see below).

The four checkout-facing instances (and future ones) dispatch their Tier-2 evidence pass to the shared read-only `agents/audit-scanner.md` (SPEC-220): its tools roster has no write path, so the propose/apply split holds mechanically in unattended cadence runs. `web-drift` is the exception, and the reason is mechanical: that roster carries no network verb, so HTTP evidence is unreachable from it, and dispatching it would answer from nothing. An instance whose evidence class lives outside the filesystem needs its own scanner or none.

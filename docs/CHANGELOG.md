...
- Command surface (`wrap land`, additive): once the merge's tree is verified, `land` now derives the landed branch's rid (the same rule `gate-ledger.sh rid` uses) and, only when that rid already has a run ledger, appends `| GATE | ship | ran | shipping pr=#<n>` -- the same line `/kit:ship` Step 8 writes, so `/kit:wrap` step 8's retro-trigger grep now fires for a spec cycle shipped through `land` too. A hand-made land with no prior ledger for its rid writes nothing, unchanged from before; a record failure prints one line and never fails the land (SPEC-317).

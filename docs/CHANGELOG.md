...
- `hooks/context-budget.sh`: a `UserPromptSubmit` hook in the `session` module that warns once per 100k-token band once a session's live context (input + cache creation + cache read of the last main-chain assistant turn) passes 200k, and clears on a drop back under budget (e.g. after `/compact`). Ported from the operator's personal, already-tested dotfiles hook. `CC_CTX_WARN`/`CC_CTX_STEP` env knobs; advisory, never blocks (SPEC-255).

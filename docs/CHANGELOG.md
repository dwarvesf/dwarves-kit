...
- Config surface (new key, BEHAVIOR CHANGE at the default): `[wrap] distill`, default `false`. A plain `/kit:wrap` now runs the landing half only (board rows, commit, merge, deploy check, tidy, pull, activity line, retro, report) and skips the distill half (the pre-step-0 candidate scan, the `before`/`after` seams, all of step 7 and its drain), reporting `Built:` and `Seam:` as `SKIPPED: distill off`. `/kit:wrap distill [repo...]` runs both halves for that call; `distill = true` in the operator `kit.toml` restores the previous every-wrap behavior. Resolves root-only. An install that relied on the seams or step 7 running on every wrap sets the key.

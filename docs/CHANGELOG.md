...
- Config surface (new key, additive, MINOR): `[wrap] autoland_carry`, default `false`, root-only, off whenever `merge_own_prs` is `false`. `true` makes `wrap apply --apply` open and merge its own stray-line and stray-commit carry PRs through `wrap merge --apply --pr` (same gate, re-merge and tree verify), and land an orphan `wrap/stray-<file>-<stamp>` branch an earlier run left without a PR when its diff is that file's carry alone. A gate refusal leaves the PR open; a failed merge or TREE MISMATCH exits 2. New env `KIT_WRAP_CARRY_CHECKS_SECS` (default 300) bounds the wait for pending checks (SPEC-322).
- Command surface (`wrap land`, additive): the fast-forward step now carries a dirty `merge=union` file's local lines across the pull, the same way `apply` already does, instead of leaving `PULL BLOCKED` for that case. A dirty non-union file is untouched and still blocks the pull exactly as before; `wrap.pull_past_dirty` is not read here (SPEC-321).
- Lane matrix (BEHAVIOR CHANGE): the `docs/WORKFLOW.md` Validate row is now `run-lite` on the normal and backfill lanes (full stays `measure-twice`). `gate-ledger.sh plan normal` and `plan backfill` list `validate  lite`, and `progress` counts it: a normal run is now 10 steps. The ship-gate does not refuse and prints nothing for it; `required normal` is unchanged (SPEC-320).
- Command surface (`gate-ledger.sh plan-record`, BEHAVIOR CHANGE): a normal or backfill `plan-record` call now needs a `validate` disposition (`--ran`, `--skipped <reason>`, or `--override <reason>`) and exits 64 naming it otherwise (SPEC-320).
- Read surface (`gate-ledger.sh progress`, BEHAVIOR CHANGE): a past normal rid with no validate line now reads one step short of complete (for example `step 4/10 (validate)` instead of `complete (9/9)`). No read-side alias credits the old self-validated `spec-validate` lines (SPEC-320).
- Read surface (`gate-ledger.sh progress`, NOTE): on the normal and backfill lanes a `Validate skipped "NEEDS REVISION: ..."` line still reads as disposed (✓validate) in `progress`, because lite phases count any disposition, ran or skipped, not just a pass; the failed verdict is only visible by reading the note, not the checkmark (SPEC-320).
- Command surface (`/kit:spec-validate`, BEHAVIOR CHANGE): it records `Validate ran` on APPROVED only; NEEDS REVISION records `Validate skipped "NEEDS REVISION: <criticals>"`, and a Reviewer 6 critical records `design-record skipped "critical: <finding>"`, so the full lane's ship-gate refuses both. `/kit:spec` step 5 and a new `/kit:execute` validation preflight dispatch it in a fresh-context, read-only subagent and the lead records `fresh agent=<id>`; `/kit:wrap` step 10 workers stop with `VALIDATE PENDING` instead of validating their own spec (SPEC-320).
- Command surface (`wrap land`, additive): once the merge's tree is verified, `land` now derives the landed branch's rid (the same rule `gate-ledger.sh rid` uses) and, only when that rid already has a run ledger with no `ship` gate line yet, appends `| GATE | ship | ran | shipping pr=#<n> via=land` -- the `via=land` tag keeps it distinguishable from a line hooks/ship-gate.sh itself gated, while `/kit:wrap` step 8's anchored retro-trigger grep still matches it, so a spec cycle shipped through `land` too gets its retro. A hand-made land with no prior ledger for its rid writes nothing; a rid whose ledger already names this PR skips as idempotent, a different PR skips as a reused-slug collision, and a record failure prints the captured error plus the exact manual command -- none of these ever fail the land (SPEC-317).
- Command surface (`/kit:spec-validate`, additive): a 7th reviewer, Sustainability Critic, advisory. A spec that leaves a long-lived job, service, external dependency, credential, or paid call behind must answer run cost, owner and liveness, dependency lifespan, retirement, and handover, or it gets a warning. A short-lived spec passes in one line. Reviewer 6 stays the only blocking reviewer (SPEC-314).
- Command surface (`/kit:spec-validate`, BEHAVIOR CHANGE, wording only): Reviewer 7's not-long-lived pass line now goes only under `## Passed`, and nothing else from that reviewer lands in Critical Issues or Warnings on the same spec, solo or co-tagged onto another reviewer's finding. A live eval showed it still co-tagging a retirement finding on a short-lived spec (SPEC-316); the quiet case is now 0/3 across three samples (SPEC-319).
- Report surface (`report-lint.sh`, BEHAVIOR CHANGE): a `**Built:**` item that is REPORTED with a `reported:` reason saying "already covers" or "nothing missing" now fails. A precedent hit that does the whole job closes as `NOTE ... covered, pointer added at <file>`, with the pointer written in-session (`commands/wrap.md` step 7b), because a REPORTED item with no work in it left step 10 nothing to build and the next session hand-rolled the tool again.
- Command surface (`wrap apply`, BEHAVIOR CHANGE at the default): a main checkout on the default branch and ahead of origin/<default> now has its stray commits pushed to a new `wrap/stray-commits-<YYYYMMDD-HHMM>` branch (the `gh pr create --head` command printed, no PR opened), and the local default branch moves back with `git reset --keep` to where it left origin, so the pull fast-forwards instead of failing as diverging. A dirty non-union tracked file leaves the branch ahead and says so. Governed by the existing `[wrap] carry_stray_lines` knob; `false` reports the count and touches nothing (SPEC-312).
- Config surface (new key, additive, MINOR): `[wrap] follow_through`, one of `off`, `lanes`, `all`, default `"off"`, so a plain `/kit:wrap` behaves as before. Resolves root-only. Any other value prints one line naming the knob and the allowed values and runs as `off`. New verb `wrap follow-mode [lanes|all]` prints the resolved mode and lanes (SPEC-310).
- Command surface (`/kit:wrap` step 0, BEHAVIOR CHANGE): a foreign writer in a repo's main checkout now stops only the writes to that checkout (steps 1, 2, 3, 5, 6 and a seam write). A step 7b build in an isolated `wrap start` worktree still runs there, and its PR stays `OPEN` (SPEC-310).
- Report surface (`report-lint.sh`, additive): a report whose first `## ` heading is `## Follow-through:` owes no `**Seam:**` line, and may close a `lane=full` item as built only as `#<pr> DRAFT` paired with a `REVIEW #<pr>` item in `Needs you`. A `## Wrap:` report is judged as before (SPEC-310).
- Config surface (new key, BEHAVIOR CHANGE at the default): `[wrap] delete_merged_remote_branches`, default `true`. `wrap apply --apply` now deletes GitHub origin branches whose same-repo PR merged at their exact origin tip, never the default branch and never a head or base of an open PR. `merge` never passes `--delete-branch`, so merged heads piled up on origin (173 on one repo). `false` reports the count and deletes nothing. Resolves root-only like the other `[wrap]` autonomy knobs.
- Config surface (new key, BEHAVIOR CHANGE at the default): `[gate] board_row_gate`, default `true`, the only `[gate]` key that defaults on. A repo with a board now blocks a commit that adds a new row without a `board-row-ok: <reason>` line. A repo opts out with `board_row_gate = false` in a committed, clean `.kit.toml`; the operator overlay can switch it off machine-wide. Resolves project > operator > kit root through `lib/gate/gate-policy.sh`, like the other gate keys.
...
- Config surface (new key, additive, MINOR): `[wrap] distill`, default `true`. A plain `/kit:wrap` runs both halves: the landing half (board rows, commit, merge, deploy check, tidy, pull, activity line, retro, report) and the distill half (the pre-step-0 candidate scan, the `before`/`after` seams, all of step 7 and its drain). Ships on because operators asked for it every session, not as a special-cased extra; the landing half always lands regardless of the knob. `distill = false` in the operator `kit.toml` restores landing-only every time, and `/kit:wrap distill [repo...]` still forces the distill half on for one call at either setting. Resolves root-only.
- Report surface (`/kit:wrap`, BEHAVIOR CHANGE at the default): `Left alone` now lists only the pass's own leftover worktree, branch, or dirty files, plus one count bullet per repo for other sessions' residue. `/kit:wrap wide [repo...]` restores the full per-item listing for that call. Actions were already session-scoped; only the report narrowed.
- Report surface (`/kit:wrap`, shape only): `Shipped`, `Left alone`, and `FYI` are markdown tables with fixed header rows instead of bullet lists. `Needs you`, `What happened`, `Built`, and `Seam` keep their shape; `report-lint.sh` is unchanged.
- Lane classifier (BEHAVIOR CHANGE): the auth hard-gate regex no longer matches the bare word `session`; it matches `session token|cookie|id|hijack|fixation|store|management|expiry` and `login|auth|user session(s)`. A task that mentions the session in the wrap or handoff sense now sizes by its other flags instead of going `full` (SPEC-296).
- `precedent find --surface inventory` matching (BEHAVIOR CHANGE): a query of three or more terms now finds rows that match all but one term, when one matched term is in the row's name. `nothing_matched` is therefore false more often for longer queries; one- and two-term queries still need every term. `_`, `-` and `/` split query terms, and stopwords (`the`, `of`, `for` ...) no longer match. Output format, `--json` keys, the `--quiet` summary line and the log line are unchanged (SPEC-308).
...
- Engine resolution (BEHAVIOR CHANGE at the `PROSE_RAG_BIN` seam): the recall engine folded into context-kit's `ctx` (`ctx index|query|hook` are the engine verbs), and `bin/prose-rag` is now an alias that resolves `$PROSE_RAG_BIN` (argv-compatible either way), then `ctx` on PATH, then `prose-rag` on PATH. `prose-rag <verb>` call sites, the flags, and the `PROSE_RAG_*` env are unchanged; `config seams` reports the same ladder, so a ctx-only install reads `filled` instead of `absent`. The no-engine exits are unchanged (`hook` exits 0, unconfigured `index` exits 0, anything else prints the install hint naming `cargo install --path <context-kit>/src/ctx` and exits 1). An install with only a pre-fold `prose-rag` binary keeps working.

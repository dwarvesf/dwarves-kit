...
- Lane classifier (BEHAVIOR CHANGE): the auth hard-gate regex no longer matches the bare word `session`; it matches `session token|cookie|id|hijack|fixation|store|management|expiry` and `login|auth|user session(s)`. A task that mentions the session in the wrap or handoff sense now sizes by its other flags instead of going `full` (SPEC-295).

...
- Command surface (`wrap land`, additive): the fast-forward step now carries a dirty `merge=union` file's local lines across the pull, the same way `apply` already does, instead of leaving `PULL BLOCKED` for that case. A dirty non-union file is untouched and still blocks the pull exactly as before; `wrap.pull_past_dirty` is not read here (SPEC-317).

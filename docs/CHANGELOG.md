...
### Added
- `tests/run-all.sh --changed [<base>]` runs only the suites the diff against `<base>` (default: the merge-base with `origin/master`) touches: suites naming a changed file's basename, changed suites themselves, `tests/test-<mod>*.sh` for `lib/<mod>/`, plus `test-meta` for anything outside `lib/`. The local pre-push check; CI keeps the full glob. `RUN_ALL_JOBS` now defaults to `auto` on macOS, where every parallel run has been green, and stays `1` on Linux until the ubuntu flake is understood.

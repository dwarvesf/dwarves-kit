...
- **A full-lane push also needs its implementation notes.** In an adopted repo (`docs/verification/README.md` present), `hooks/ship-gate.sh` refuses a `full`-lane push unless `docs/implementation-notes/<slug>.md` or `docs/implementation-notes/SPEC-NNN-<slug>.md` is committed in `HEAD` with at least one line beyond its `# ` title. A change with no deviations records one line: `No deviations; matches the spec verbatim`. The gap prints as `MISSING-NOTES`; a logged `gate-ledger.sh override <rid> impl-notes "<reason>"` clears it. A repo without the marker gets an `[advisory]` line instead. The rule lives in the hook, not in `gate-ledger.sh check`, because `check` has no repo root: subagent-run lanes never see the prompt-level notes instruction, so the push is where it is checked.

...
CC_SI_REVIEWER_CMD="sleep 2; cat $TMP/env.json" bash "$HOOK" < "$PAY"; rc=$?
...
appeared=0; for _ in $(seq 1 40); do [[ -f "$CC_SI_PROPOSALS_DIR/async-skill/SKILL.md" ]] && { appeared=1; break; }; sleep 0.25; done
...
rm -rf "$CC_SI_PROPOSALS_DIR" "$CC_SI_STATE_DIR" 2>/dev/null
CLAUDE_REVIEWING=1 CC_SI_REVIEWER_CMD="touch $TMP/REENT; cat $TMP/env.json" bash "$HOOK" < "$PAY"; rc=$?
sleep 0.6
...
echo "[4] disabled: CC_SI_ENABLED=false -> hook does nothing (negative control)"
rm -rf "$CC_SI_PROPOSALS_DIR" "$CC_SI_STATE_DIR" 2>/dev/null
CC_SI_ENABLED=false CC_SI_REVIEWER_CMD="touch $TMP/DISABLED; cat $TMP/env.json" bash "$HOOK" < "$PAY"; rc=$?
sleep 0.6

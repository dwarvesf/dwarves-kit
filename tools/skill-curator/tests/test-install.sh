...
ncsi(){ jq '[.hooks[]?[]?.hooks[]?.command | select(test("skill-curator"))] | length' "$CC_SI_SETTINGS" 2>/dev/null; }
...
allasync="$(jq -r '[.hooks[]?[]?.hooks[]?|select(.command|test("skill-curator"))|.async]|all(.==true)' "$CC_SI_SETTINGS")"

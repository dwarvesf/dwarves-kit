...
export CC_SI_STATE_DIR="$TMP/state" CC_SI_PROPOSALS_DIR="$TMP/proposals" CC_SI_SKILLS_DIR="$TMP/skills"

mkdraft(){ mkdir -p "$CC_SI_PROPOSALS_DIR/$1"; printf '%s\n' "$2" > "$CC_SI_PROPOSALS_DIR/$1/SKILL.md"; }
GOOD='---
...
if [[ -f "$CC_SI_SKILLS_DIR/deploy-via-wrangler/SKILL.md" && ! -d "$CC_SI_PROPOSALS_DIR/deploy-via-wrangler" ]]; then ok "promoted + proposal removed"; else no "promote did not move"; fi
...
if [[ $rc -eq 4 && -d "$CC_SI_PROPOSALS_DIR/deploy-via-wrangler" ]]; then ok "refused overwrite (rc=4), draft still staged"; else no "did not refuse overwrite (rc=$rc)"; fi

echo "[4] promote --force replaces but backs up the old (never rm)"
printf 'LIVE-MARKER\n' >> "$CC_SI_SKILLS_DIR/deploy-via-wrangler/SKILL.md"
"$CLI" promote deploy-via-wrangler --force >/dev/null 2>&1
if [[ -f "$CC_SI_SKILLS_DIR/deploy-via-wrangler/SKILL.md" ]] && ! grep -q LIVE-MARKER "$CC_SI_SKILLS_DIR/deploy-via-wrangler/SKILL.md" && [[ -n "$(find "$CC_SI_PROPOSALS_DIR/_replaced" -name SKILL.md 2>/dev/null)" ]] && grep -rq LIVE-MARKER "$CC_SI_PROPOSALS_DIR/_replaced" 2>/dev/null; then
...
if [[ $rc -eq 3 && ! -d "$CC_SI_SKILLS_DIR/secret-draft" && -d "$CC_SI_PROPOSALS_DIR/secret-draft" ]]; then ok "secret draft refused (rc=3), not promoted"; else no "secret promote not refused (rc=$rc)"; fi

echo "[6] reject moves to _rejected/ (recoverable, not deleted)"
"$CLI" reject other-skill >/dev/null
if [[ ! -d "$CC_SI_PROPOSALS_DIR/other-skill" && -f "$CC_SI_PROPOSALS_DIR/_rejected/other-skill/SKILL.md" ]]; then ok "rejected -> _rejected/ (recoverable)"; else no "reject did not move to _rejected"; fi

echo "[7] reject/promote left unrelated live skills untouched"
mkdir -p "$CC_SI_SKILLS_DIR/unrelated"; printf 'KEEP\n' > "$CC_SI_SKILLS_DIR/unrelated/SKILL.md"
"$CLI" reject secret-draft >/dev/null
if grep -q KEEP "$CC_SI_SKILLS_DIR/unrelated/SKILL.md"; then ok "unrelated skill untouched"; else no "unrelated skill changed"; fi

echo "[8] auto-promote OFF by default -> promotes nothing even for an eligible draft"
rm -rf "$CC_SI_PROPOSALS_DIR" 2>/dev/null
mkdir -p "$CC_SI_SKILLS_DIR/deploy-via-wrangler"   # umbrella exists
mkdraft refadd "$(printf -- '---\nname: refadd\ncc-si-kind: references-add\ncc-si-umbrella: deploy-via-wrangler\ncc-si-path: references/extra.md\n---\n# add')"
out_off="$(CC_SI_AUTO_PROMOTE=false "$CLI" auto 2>&1)"
if grep -q 'disabled' <<<"$out_off" && [[ ! -f "$CC_SI_SKILLS_DIR/deploy-via-wrangler/references/extra.md" ]]; then ok "auto off -> nothing promoted"; else no "auto promoted while off"; fi
...
out_on="$(CC_SI_AUTO_PROMOTE=true "$CLI" auto 2>&1)"
if [[ -f "$CC_SI_SKILLS_DIR/deploy-via-wrangler/references/extra.md" && ! -d "$CC_SI_SKILLS_DIR/newskill" && -d "$CC_SI_PROPOSALS_DIR/newskill" ]]; then

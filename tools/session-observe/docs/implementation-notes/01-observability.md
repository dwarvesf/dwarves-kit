...
## 2026-06-15 cc-vps-report: heartbeat-only by default, snapshot opt-in (agent-silent false-alarm)
- Context: the SG-05 weekly bridge fired a false `agent-silent` CRIT on `cc-air` at 19:50 (2026-06-15), the first weekly run after #343 deployed.
- Decision / Change: inverted the bridge's default. The heartbeat GET is now the always-run default action; the `/v1/snapshot` POST moved behind an opt-in `--snapshot` flag (default OFF). Removed `--no-heartbeat` (the snapshot is no longer the default leg, so a "snapshot only" flag made no sense). `--dry-run` still exercises the snapshot signer offline regardless of `--snapshot`. `cc-intel-weekly` now calls `cc-vps-report` with no flag (heartbeat-only) and reads only the HB token.
- Why: `/v1/snapshot` UPSERTS a `hosts` row, and the prober's hardcoded `AGENT_SILENT_THRESHOLD_SEC = 600` (`tools/vps-mon/worker/src/prober.ts`) CRITs any host whose `last_seen` is older than 600s. A WEEKLY pusher's host row is stale 6 days out of 7, so the snapshot path guarantees a weekly false CRIT. The heartbeat (interval 604800 + grace 86400) does not touch the `hosts` table and is the correct weekly liveness signal. The snapshot + signer are kept (valid + tested) for a future per-host exempt in vps-mon.
- Alternatives considered: (a) raise/per-host the 600s threshold in vps-mon, rejected for now, that is a vps-mon design change with broader blast radius; the client-side default flip is the surgical fix and leaves the exempt as the clean long-term path. (b) delete the snapshot code, rejected, it is correct and tested, only mis-scheduled; keep it opt-in.
- Impact: `bin/cc-vps-report` (main() restructured, `--snapshot` replaces `--no-heartbeat`, docstring), `tools/cc-intel/deploy/macos/cc-intel-weekly` (heartbeat-only call), proof-of-done (post-ship fix note + reproduce). Mitigation already applied live: deleted the `cc-air` host row + acked the alert. Signer test unchanged, still 6/6 green.

...
RC=0
grep -qE '^\s*collect\)' "$KIT_DIR/lib/reflect/weekend-batch.sh" || RC=1
grep -qE '^\s*mark-paid\)' "$KIT_DIR/lib/reflect/weekend-batch.sh" || RC=1
assert "weekend-batch: lib/reflect/weekend-batch.sh exposes 'collect' + 'mark-paid' verbs" $RC

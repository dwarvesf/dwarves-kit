...
# Capped at 4 on purpose, not set to the core count. A suite here is not one
# process: the heaviest spawn 100-800 subprocesses each. One worker per core
# therefore oversubscribes the machine several times over, and the heavy suites
# starve. Measured on a 10-core M4: -P 10 pushed test-meta, test-config-seams
# and test-orchestrate-wavefront past the 300s ceiling, suites that pass in well
# under it when the box is not thrashing.
_cores_capped() { c="$(_cores)"; [ "$c" -gt 4 ] && c=4; echo "$c"; }
JOBS="${RUN_ALL_JOBS:-$(_cores_capped)}"

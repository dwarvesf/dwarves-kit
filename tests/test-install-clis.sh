...
for cli in session worktree-provision prose-rag; do
...
assert_true "shim targets the stable bin/ entrypoint" \
  "$(grep -q "$H1/.claude/dwarves-kit/bin/session" "$H1/.local/bin/session"; echo $?)"
...
echo "== board module exposes the board CLI (promote = the human gate, ex add-backlog) =="
H6="$(mktemp -d)"
HOME="$H6" bash "$KIT_DIR/install.sh" --with board >/tmp/kitcli-h6.log 2>&1
assert_true "board shim present via board module" "$([ -x "$H6/.local/bin/board" ]; echo $?)"
out="$(cd "$(mktemp -d)" && HOME="$H6" "$H6/.local/bin/board" promote 2>&1)"
assert_true "board promote runs (empty repo -> no staged candidates)" "$(grep -qE 'no staged candidates|nothing staged' <<<"$out"; echo $?)"

echo "== NC: spine-only install exposes no CLIs =="
H2="$(mktemp -d)"
HOME="$H2" bash "$KIT_DIR/install.sh" --prune >/tmp/kitcli-h2.log 2>&1
assert_true "no session shim without the session module" "$([ ! -e "$H2/.local/bin/session" ]; echo $?)"
...
printf '#!/bin/sh\necho user-owned\n' > "$H3/.local/bin/session"
chmod +x "$H3/.local/bin/session"
HOME="$H3" bash "$KIT_DIR/install.sh" --with session >/tmp/kitcli-h3.log 2>&1
assert_true "user-owned session untouched" "$(grep -q 'user-owned' "$H3/.local/bin/session"; echo $?)"
...
ln -s /nonexistent/old-snapshot/session "$H4/.local/bin/session"

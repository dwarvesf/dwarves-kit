...
If the diff touches Cloudflare (Workers, Durable Objects, D1, R2, KV, Queues, Workflows, wrangler config), also check it against `docs/impl-playbook/cloudflare.md`: `wrangler.jsonc` vs `.toml` config gating, `wrangler versions secret put` (not the plain form) once gradual deployments are in use, D1's lack of interactive transactions, and KV's eventual-consistency window.

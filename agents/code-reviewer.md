...
- Is each test at the right layer per `docs/impl-playbook/testing-strategy.md` (Fowler's pyramid): a pure-function/single-module case should be unit, not e2e; only the golden path plus the highest-cost failure mode earns end-to-end.
- Are edge cases from the spec tested? Cross-check against `docs/impl-playbook/test-case-design.md`: boundary values tested at the edges (not one interior value), and any case with 3+ interacting conditions covered by more than a single happy-path assertion.

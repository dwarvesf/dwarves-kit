...
**Built:** <label> ENHANCE <home>: <file, insertion point> (<commit> | staged)
   -- and/or -- <label> NEW (precedent: nothing matched): <path> (<commit> | staged)
...
- **`**Built:**` is REQUIRED, and the lint fails without it.** One line, after `FYI`, reporting step 7b's outcome as exactly one of three: each candidate as `<label> ENHANCE <home>: <file, insertion point>` or `<label> NEW (precedent: nothing matched): <path>`, each followed by its commit or `staged`; `NOTHING: no candidates` when the scan ran and found none; or `SKIPPED: <why>` when the step did not run. The ENHANCE and NEW tokens are what the lint checks for on a non-empty line, because they are the slot that forces naming the existing tool a candidate joins; a `Built:` that names only a path and a commit is the session's deliverable wearing 7b's label. The three must stay distinguishable, and `SKIPPED: nothing to build` collapses two of them. A skipped 7b and an empty 7b used to look identical, which is how the step that builds instead of proposing got silently dropped from a whole session. This line is the trace every other step already leaves. The lint also WARNS (never fails) when a `NEW (precedent: nothing matched)` item's path exists and its files speak CDP (`session.Runtime.evaluate` and siblings): that candidate already has a home in browser-harness-js's learnings, not a sibling script here.

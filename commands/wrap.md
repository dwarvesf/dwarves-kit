...
Sub-step `b` BUILDS what it can rather than proposing it, and it builds through the lane, never around it. A tiny-lane candidate is built and verified here, because a wrong call costs one revert and a staged row that waits for a yes costs a round trip on work the operator already asked for. Anything heavier is staged with its goal drafted, because a spec, a review, or a root cause does not fit in the minutes an operator gave you to close the session, and a change that skips them is unreviewed and undocumented wherever it lands. Sub-step `c` writes one memory note and never a board row.
...
**Built:** <label> ENHANCE <home>: <file, insertion point> (lane=tiny, verified: <check>, <commit>)
   -- and/or -- <label> NEW (precedent: nothing matched): <path> (lane=tiny, verified: <check>, <commit>)
   -- and/or -- <label> ENHANCE <home>: <file, insertion point> (lane=normal, staged + goal drafted: <path>)
   -- and/or -- <label> ENHANCE <home>: <file, insertion point> (lane=normal, staged: build_candidates off)
...
- **`**Built:**` is REQUIRED, and the lint fails without it.** One line, after `FYI`, reporting step 7b's outcome as exactly one of three: each candidate as `<label> ENHANCE <home>: <file, insertion point>` or `<label> NEW (precedent: nothing matched): <path>`, each followed by its LANE and how it closed, one of `(lane=tiny, verified: <the check that ran>, <commit>)` for one built here, `(lane=<lane>, staged + goal drafted: <path>)` for one routed on, or `(lane=<lane>, staged: build_candidates off)` when the knob stayed wrap's hand; `NOTHING: no candidates` when the scan ran and found none; or `SKIPPED: <why>` when the step did not run. The lane and the check are in the line because a candidate built at session close is invisible everywhere else: naming the lane says which discipline it passed, and naming the check says the build was proven rather than asserted. The ENHANCE and NEW tokens are what the lint checks for on a non-empty line, because they are the slot that forces naming the existing tool a candidate joins; a `Built:` that names only a path and a commit is the session's deliverable wearing 7b's label. The three must stay distinguishable, and `SKIPPED: nothing to build` collapses two of them. A skipped 7b and an empty 7b used to look identical, which is how the step that builds instead of proposing got silently dropped from a whole session. This line is the trace every other step already leaves.

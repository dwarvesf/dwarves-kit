...
- `wrap.pull_past_dirty` (default `false`) governs the third way a repo stays behind: a sibling session's dirty TRACKED file that git refuses to overwrite. `false` reports `FAILED` and leaves the checkout behind. `true` stashes only the blocking files under a run-named stash, pulls, and pops that stash by ref; a pop conflict prints `PULLED, POP CONFLICT: <files>, stash <name> kept` and exits 2, leaving the markers and the stash for the operator. Untracked files, a dirty index, and pre-existing stashes are out of its reach at either setting. Name the knob in `FYI` whenever it changed what the pull did.

...
`wrap.build_lanes` sizes step 7b rather than authorizing it: a space-separated list of the lanes step 7b builds inline. The default `"tiny"` is today's behavior. It resolves root-only for the same reason as the three above, because widening it widens what wrap writes.

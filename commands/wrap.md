...
- **`full`, whatever the list says.** A full-lane candidate is never built inline: a spec and a review are the point of that lane and neither fits in the minutes an operator gave you to close the session. It does not stage either. File it on the home repo's BOARD as a queued row, then draft its goal:

  ```bash
  bin/board capture "<title> #<tags>" -b "<intent, precedent hit, lane=full, source session>" --backlog-file <home>/_meta/BACKLOG.md
  ```

  When `capture` exits 2 because the home repo has no board, run `bin/board init` from that home repo's root (idempotent: it scaffolds an empty `_meta/BACKLOG.md` plus the `_meta/board` shim, the same thing `/kit:onboard` offers) and run the capture again; any other non-zero exit is reported as `(lane=full, capture failed: <first line of stderr>)` and the goal draft is still written.

  `capture` detects the repo's own ID prefix, mints the next id against the board file AND its git history (it fetches origin first, so an id another session already pushed is never reused), and inserts `| <ID-NNN> | <title> #tags | <notes> | queued |` under the Active queue header. It prints the id it minted. Then draft the goal at `.claude/goals/<slug>.md` in that home repo, the same six-section shape as below, and name that path in the row's notes so `/kit:assign` finds it. Report it with `(lane=full, filed: <repo> <ID-NNN>, goal drafted: <path>)`. A board row is a queue an operator actually reads; a staged row is not, and the one estate board holding staged rows has drained none of them since May.
- **Any other lane not in the list.** Do not build it here. Two writes instead, in this order: stage the row with `bin/wrap stage "<title>" "<intent>" "<home>" --repo <the checkout of that home repo>`, then draft the goal for it, a six-section operating directive (Context-to-read / Constraints / Operating rules / Validation loop / Done-when / Pause-if) at `.claude/goals/<slug>.md` in that home repo, the shape `/kit:assign` writes and the shape the next session runs. Name the lane and the `Done =` line in the draft. Report it with `(lane=<lane>, staged + goal drafted: <path>)`.
...
- **A row without a goal pointer is never drained.** `wrap stage` writes prose (Intent, Approach, Tags, Home) and `queue run` needs a pointer path. A row this pass staged through the lane route above carries the draft it wrote at `.claude/goals/<slug>.md`, which is that pointer. A row from the knob-false path or an earlier session carries none: report each in `FYI` as a `STATE` row with the reason and the step that closes it, `/kit:assign`, which turns a board row into a goal draft. Do not invent a pointer to make a row runnable.
- **Report what was dispatched, not what will happen.** The queue is asynchronous and its journal is the record. Name the tsv, the row count, and the journal path in `FYI` as a `STATE` row; never claim an outcome the queue has not written yet.
...
- STAGED <label> NEW (precedent: nothing matched): <path> (lane=bug, staged + goal drafted: <path>)
- FILED <label> NEW (precedent: nothing matched): <path> (lane=full, filed: <repo> <ID-NNN>, goal drafted: <path>)
- STAGED <label> NEW (precedent: nothing matched): <path> (lane=normal, staged: build_candidates off)
...
| STATE | <a fact the operator will meet next time: a knob value, a dirty checkout, a file that exists only on this machine> | <file or board, when the fact is a captured follow-up> |
...
- **Every `FYI` row of the second kind NAMES ITS HOME** in the `Home` column, the file or board where the follow-up already lives. That is this section's own admission test. A row carrying future work with an empty `Home` cell is a task hiding in the read-only lane, which is how a good idea reaches the end of a session and dies there; give it a home first, then report the path. A row that needs a decision, a command, or a review FROM THE OPERATOR was misfiled and belongs in `Needs you` instead.

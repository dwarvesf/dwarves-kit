...
Each governs exactly one step: `merge_own_prs` step 3, `tidy_worktrees` step 5, `build_candidates` step 7b. The shipped defaults are all `true`, so wrap acts. `build_candidates`, `build_lanes`, and `drain_staged` size a step inside the distill half, so with `wrap.distill` off and no `distill` argument they have nothing to govern. Every one of the three authorizes a write, which is why all three resolve with `kit_config_get_root` and never from a project `.kit.toml`. A `false` turns that step's action into a report line; it never turns the step off, and it never relaxes a refusal the tools make on their own. Name every knob read as `false` in the report's `FYI`, so a step that stayed its hand says why.
...
- <label> ENHANCE <home>: <file, insertion point> (lane=normal, verified: <check>, <PR>)
- <label> NEW (precedent: nothing matched): <path> (lane=bug, staged + goal drafted: <path>)
- <label> NEW (precedent: nothing matched): <path> (lane=full, filed: <repo> <ID-NNN>, goal drafted: <path>)
- <label> NEW (precedent: nothing matched): <path> (lane=normal, staged: build_candidates off)

...
- `apply` reports STRAY LINES on the main checkout when the default branch is checked out: for each dirty `merge=union` file, the lines neither origin/<default> nor HEAD holds. A session that ran `board set` or `wrap log` in the shared checkout leaves exactly these, and the pull carry keeps them local forever. The dry run prints `WOULD carry N stray lines in <file> onto a branch`. `--apply` commits origin's version plus those lines to a new `wrap/stray-<file-slug>-<YYYYMMDD-HHMM>` branch in a scratch worktree, pushes it, and prints `gh pr create --head <branch>`; it opens no PR and never touches the working copy. Relay that command under `Needs you`. An origin `wrap/stray-<file-slug>-*` branch that already exists skips the file. `wrap.carry_stray_lines` (default `true`) false reports the count and carries nothing; name it in `FYI` as a `STATE` row.

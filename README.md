...
dwarves-kit/
  ...
  lib/queue.sh                  Overnight queue LAUNCHER (SPEC-146): drives REAL interactive Claude Code `/goal` sessions via terminal-mux send-keys (NOT headless `claude -p`, to sidestep the AUTH/KILL-CLASS risk). `run <src.tsv>` (rows `slug<TAB>repo<TAB>pointer`) opens a fresh `tmux`/`cmux` window per queued mega, types `/goal <pointer>` + Enter, polls `capture-pane` for the line-anchored completion marker (`RUNNER_DONE`/`RUNNER_GATED`), journals each verdict to `queue-journal.tsv`, and stops the night after two consecutive `error` megas. Flags: `--dry-run`, `--max-megas N`, `--from-boards` (sub-goal 04's `board queue` emit). CONSUMER config: `TERMINAL_MUX`/`MUX_CMD`/`QUEUE_CLAUDE_*`/`QUEUE_JOURNAL`/`QUEUE_*_SECS`. Aliased as `orchestrate.sh queue <src>`. Idempotent (a `done` slug is skipped on re-run)
